 ...
 **  @date               : 2024/02/10
 **  @vendor             : isoft
 **  @description        : Mcu header file for Irq api declarations
 **
 ***********************************************************************************************************************/
...
/*=======[M A C R O S]=====================================================*/
#define SRC_SRE_BIT  (10u)
#define SRC_CLRR_BIT (25u)
...
/* set the bit7 ICxxx.MKxxx(request mask) */
#define OS_INTERRUPT_ENABLEREQ(src) (*(uint16 volatile*)(src)) = ((*(uint16 volatile*)(src)) | 0x00000400u)

/* Clear the bit10 SRE(source request enable) */
#define OS_INTERRUPT_DISABLEREQ(src) (*(uint32 volatile*)(src)) = ((*(uint32 volatile*)(src)) & 0xFFFFFBFFu)
...
#define OS_GETISRSTR(isrId) OS_INTERRUPT_CHKREQ(Os_IsrCfg[isrId].IsrSrc)

/* Clear the status register of the ISR id */
#define OS_CLRISRSTR(isrId) OS_INTERRUPT_CLEARREQ(Os_IsrCfg[isrId].IsrSrc)
...
#define Os_Hal_RemoteCall(coreId)        \
    {                                    \
        OS_INTERRUPT_SRB_SETREQ(Os_McuGetCorePhyID(coreId)); \
    }
/* PRQA S 3472, 3458 -- */

/* PRQA S 3412 ++ */ /* VL_Os_3412 */
#define OS_Hal_DisableIntApp(src) \
    OS_INTERRUPT_DISABLEREQ(src); \
    OS_INTERRUPT_CLEARREQ(src);   \
    OS_INTERRUPT_SET_SRPN_ZERO(src);
/* PRQA S 3412 -- */
 ...
 */
/******************************************************************************/
extern void Os_McuInterruptInstall(const Os_IsrCfgType* IsrCfg, const Os_IntInstallType* IntInstall);
 ...
 * @return      uint32: The current running IRQ.
 * @reentrant   TRUE
 * @synchronous FALSE
 * @trace        -
 */
 ...
 * @return      uint32: The current running Exception.
 * @reentrant   TRUE
 * @synchronous FALSE
 * @trace        -
 */
