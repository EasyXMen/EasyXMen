...
/* CRC16/ARC Standard Init value*/
#define CRC_INITVALUE16ARC 0x0000u
/* CRC16/ARC Standard value*/
#define CRC_XORVALUE16ARC 0x0u
#if (CRC_RUNTIME == CRC16ARC_MODE)
/*CRC16/ARC Standard Generator Polynomial*/
...
#define CRC_XORVALUE32 0xFFFFFFFFuL
...
/* CRC32 xor value*/
#define CRC_XORVALUE32P4 0xFFFFFFFFuL
...
/* CRC64 Init value*/
#define CRC_INITVALUE64 0xFFFFFFFFFFFFFFFFuLL
/* CRC64 xor value*/
#define CRC_XORVALUE64 0xFFFFFFFFFFFFFFFFuLL
...
#define CRC_MSB_BYTE                0x80u
#define CRC_MSB_WORD                0x8000u
#define CRC_MSB_DWORD               0x80000000u
#define CRC_MSB_ULL                 0x8000000000000000uLL
#define CRC_BIT_LENGTH              0x1u
#define CRC_BIT_LENGTH8             0x8u
#define CRC_BIT_LENGTH24            0x18u
#define CRC_BIT_LENGTH56            0x38u
#define CRC_MASK_CPU_32_WITH_16_ADR 0xffu
#define CRC_TABLE_NUM               0x0100u
/* ================================================ type definitions ================================================ */
 ...
 * @trace         -
 */
CRC_LOCAL uint8 Crc_GetReflectionOfChar(uint8 input);
#endif
#if ((STD_ON == CRC16ARC_ALGORITHM) && (CRC_RUNTIME == CRC16ARC_MODE))
/**
 * @brief         get the reflection of short type data
 * @param[in]     input : input data of short
 * @return        uint16
 * @reentrant     TRUE
 * @synchronous   TRUE
 * @trace         -
 */
CRC_LOCAL uint16 Crc_GetReflectionOfShort(uint16 input);
#endif
#if (                                                            \
    ((STD_ON == CRC32_ALGORITHM) && (CRC_RUNTIME == CRC32_MODE)) \
    || ((STD_ON == CRC32P4_ALGORITHM) && (CRC_RUNTIME == CRC32P4_MODE)))
 ...
 * @trace         -
 */
CRC_LOCAL uint32 Crc_GetReflectionOfInt(uint32 input);
#endif
#if ((STD_ON == CRC64_ALGORITHM) && (CRC_RUNTIME == CRC64_MODE))
/**
 * @brief         get the reflection of uint64 type data
 * @param[in]     input : input data of uint64
 * @return        uint64
 * @reentrant     TRUE
 * @synchronous   TRUE
 * @trace         -
 */
CRC_LOCAL uint64 Crc_GetReflectionOfLonglong(uint64 input);
...
/* clang-format off */
/**
 * Look-up table in Crc_CalculateCRC32
 */
CRC_LOCAL const uint32 Crc_CalCrc32Tab[CRC_TABLE_NUM] = { /*PRQA S 3218*/ /*VL_Crc_Scope*/
...
#include "Crc_MemMap.h"
/**
 * Look-up table in Crc_CalculateCRC32P4
 */
CRC_LOCAL const uint32 Crc_CalCrc32p4Tab[CRC_TABLE_NUM] = { /*PRQA S 3218*/ /*VL_Crc_Scope*/
...
#include "Crc_MemMap.h"
/**
 * Look-up table in Crc_CalculateCRC16
 */
CRC_LOCAL const uint16 Crc_CalCrc16Tab[CRC_TABLE_NUM] = { /*PRQA S 3218*/ /*VL_Crc_Scope*/
...
#if ((STD_ON == CRC16ARC_ALGORITHM) && (CRC_TABLE == CRC16ARC_MODE))
#define CRC_START_SEC_CONST_16
#include "Crc_MemMap.h"
/**
 * Look-up table in Crc_CalculateCRC16ARC
 */
CRC_LOCAL const uint16 Crc_CalCrc16arcTab[CRC_TABLE_NUM] = { /*PRQA S 3218*/ /*VL_Crc_Scope*/
...
#include "Crc_MemMap.h"
/**
 * Look-up table in Crc_CalculateCRC8
 */
CRC_LOCAL const uint8 Crc_CalCrc8Tab[CRC_TABLE_NUM] = { /*PRQA S 3218*/ /*VL_Crc_Scope*/
...
#include "Crc_MemMap.h"
/**
 * Look-up table in Crc_CalculateCRC8H2F
 */
CRC_LOCAL const uint8 Crc_CalCrc8h2fTab[CRC_TABLE_NUM] = { /*PRQA S 3218*/ /*VL_Crc_Scope*/
...
#include "Crc_MemMap.h"
/**
 * Look-up table in Crc_CalculateCRC64
 */
CRC_LOCAL const uint64 Crc_CalCrc64Tab[CRC_TABLE_NUM] = { /*PRQA S 3218*/ /*VL_Crc_Scope*/
...
uint8 Crc_CalculateCRC8(const uint8* Crc_DataPtr, uint32 Crc_Length, uint8 Crc_StartValue8, boolean Crc_IsFirstCall)
{
    ...
    if (Crc_IsFirstCall)
    {
        crc_value = CRC_INITVALUE8;
    }
    else
    {
        crc_value = Crc_StartValue8 & CRC_MASK_CPU_32_WITH_16_ADR; /* PRQA S 2985*/ /* VL_Crc_Redudant */

        crc_value ^= CRC_XORVALUE8;
    }
...
/* check if it is runtime method to calculate */
    ...
    for (i = 0u; i < Crc_Length; i++)
    {
        ...
        crc_value ^= Crc_DataPtr[i]; /* PRQA S 2824*/ /* VL_Crc_NullPointer */

        for (j = 0u; j < CRC_BIT_LENGTH8; j++)
        {
            /*check if MSB is 1*/
            if ((crc_value & CRC_MSB_BYTE) > 0x0u)
            {
                crc_value <<= CRC_BIT_LENGTH; /* PRQA S 2911*/ /* VL_Crc_LeftShift */
                crc_value ^= CRC_POLY8;
            }
            else
            {
                crc_value <<= CRC_BIT_LENGTH;
            }
        }
    }
/* check if it is table method to calculate */
    ...
    for (i = 0u; i < Crc_Length; i++)
    {
        crc_value = (uint8)Crc_CalCrc8Tab[(crc_value ^ (uint8)Crc_DataPtr[i])];
    }
...
/* check if it is hardware method to calculate */
    ...
    return crc_value & CRC_MASK_CPU_32_WITH_16_ADR; /* PRQA S 2985 */ /* VL_Crc_Redudant */
}
#endif
    ...
    boolean      Crc_IsFirstCall)
{
    ...
    if (Crc_IsFirstCall)
    {
        crc_value = CRC_INITVALUE8H2F;
    }
    else
    {
        crc_value = Crc_StartValue8H2F & CRC_MASK_CPU_32_WITH_16_ADR; /* PRQA S 2985 */ /* VL_Crc_Redudant */
        crc_value ^= CRC_XORVALUE8H2F;
    }
...
/* check if it is table method to calculate */
    ...
    for (i = 0u; i < Crc_Length; i++)
    {
        crc_value = (uint8)Crc_CalCrc8h2fTab[(crc_value ^ (uint8)Crc_DataPtr[i])];
    }
...
/* check if it is runtime method to calculate */
        ...
        {
            /* current CRC8 XOR data in */
            crc_value ^= (uint16)((uint16)Crc_DataPtr[i] << CRC_BIT_LENGTH8);

            for (j = 0u; j < CRC_BIT_LENGTH8; j++)
            {
                /*check if MSB is 1*/
                if ((crc_value & CRC_MSB_WORD) > 0x0u)
                {
                    crc_value <<= CRC_BIT_LENGTH; /* PRQA S 2911*/ /* VL_Crc_LeftShift */
                    crc_value ^= CRC_POLY16;
                }
                else
                {
                    crc_value <<= CRC_BIT_LENGTH;
        ...
        for (i = 0u; i < Crc_Length; i++)
        {
            /*PRQA S 3120 ++ */ /*VL_QAC_MagicNum*/ /* PRQA S 2985,2986 ++ */ /* VL_Crc_Redudant */
            crc_value = Crc_CalCrc16Tab[(((crc_value & 0xFF00u) >> 8u) ^ ((uint16)Crc_DataPtr[i])) & 0xFFu]
                        ^ ((uint16)((crc_value & 0x00FFu) << 8u));
            /*PRQA S 3120 -- */ /* PRQA S 2985,2986 -- */
        }
...
/* check if it is hardware method to calculate */
        ...
        crc_value ^= CRC_XORVALUE16; /* PRQA S 2985 */ /* VL_Crc_Redudant */
    }
    return crc_value;
}
#endif
 ...
 */
uint16
    Crc_CalculateCRC16ARC(const uint8* Crc_DataPtr, uint32 Crc_Length, uint16 Crc_StartValue16, boolean Crc_IsFirstCall)
{
    uint16 crc_value;
#if (CRC_HARDWARE != CRC16ARC_MODE)
    uint32 i;
#endif
#if (CRC_RUNTIME == CRC16ARC_MODE)
    uint8 j;
    ...
    if (Crc_IsFirstCall)
    {
        crc_value = CRC_INITVALUE16ARC;
    }
    else
    {
        crc_value = Crc_StartValue16;
        crc_value ^= CRC_XORVALUE16ARC; /* PRQA S 2985 */ /* VL_Crc_Redudant */
#if (CRC_RUNTIME == CRC16ARC_MODE)
        crc_value = Crc_GetReflectionOfShort(crc_value);
#endif
    }
...
/* check if it is runtime method to calculate */
    ...
    for (i = 0u; i < Crc_Length; i++)
    {
        ...
        /* PRQA S 2824 ++ */ /* VL_Crc_NullPointer */
        crc_value ^= (uint16)((uint16)Crc_GetReflectionOfChar(Crc_DataPtr[i]) << CRC_BIT_LENGTH8);
        /* PRQA S 2824 -- */
        for (j = 0u; j < CRC_BIT_LENGTH8; j++)
        {
            /*check if MSB is 1*/
...
/* check if it is table method to calculate */
#elif (CRC_TABLE == CRC16ARC_MODE)
    for (i = 0u; i < Crc_Length; i++)
    {
        /*PRQA S 3120 ++ */ /*VL_QAC_MagicNum*/
        crc_value = Crc_CalCrc16arcTab[(((crc_value) ^ ((uint16)Crc_DataPtr[i]))) & 0xffu] ^ (crc_value >> 0x8u);
        /*PRQA S 3120 -- */
    }
    /* check if it is hardware method to calculate */
#elif (CRC_HARDWARE == CRC16ARC_MODE)
    Crc_CalculateCRC16ARCByHardware(Crc_DataPtr, Crc_Length, Crc_StartValue16, Crc_IsFirstCall);
#endif
...
uint32 Crc_CalculateCRC32(const uint8* Crc_DataPtr, uint32 Crc_Length, uint32 Crc_StartValue32, boolean Crc_IsFirstCall)
{
    ...
    if (Crc_IsFirstCall)
    {
        crc_value = CRC_INITVALUE32;
    }
    else
    {
        crc_value = Crc_StartValue32;
        crc_value ^= CRC_XORVALUE32;
#if (CRC_RUNTIME == CRC32_MODE)
        crc_value = Crc_GetReflectionOfInt(crc_value);
#endif
    }
...
/* check if it is runtime method to calculate */
    ...
    for (i = 0u; i < Crc_Length; i++)
    {
        /* current CRC32 high byte XOR data in(LSB first),left move 24 bit */
        /* PRQA S 2824 ++ */ /* VL_Crc_NullPointer */
        crc_value ^= ((uint32)Crc_GetReflectionOfChar(Crc_DataPtr[i]) << CRC_BIT_LENGTH24);
        /* PRQA S 2824 -- */

        for (j = 0u; j < CRC_BIT_LENGTH8; j++)
        {
            /*check if MSB is 1*/
            if ((crc_value & CRC_MSB_DWORD) > 0x0u)
            {
                crc_value <<= CRC_BIT_LENGTH; /* PRQA S 2921*/ /* VL_Crc_LeftShift */
                crc_value ^= CRC_POLY32;
            }
            else
            {
                crc_value <<= CRC_BIT_LENGTH;
...
/* check if it is table method to calculate */
    ...
    {
        /*PRQA S 3120 ++ */ /*VL_QAC_MagicNum*/
        crc_value = Crc_CalCrc32Tab[(crc_value ^ (uint32)Crc_DataPtr[i]) & 0xffuL] ^ (crc_value >> 0x8u);
        /*PRQA S 3120 -- */
    }
    /* check if it is hardware method to calculate */
    ...
    Crc_CalculateCRC32P4(const uint8* Crc_DataPtr, uint32 Crc_Length, uint32 Crc_StartValue32, boolean Crc_IsFirstCall)
{
    ...
    if (Crc_IsFirstCall)
    {
        crc_value = CRC_INITVALUE32P4;
    }
    else
    {
        crc_value = Crc_StartValue32;
        crc_value ^= CRC_XORVALUE32P4;
#if (CRC_RUNTIME == CRC32P4_MODE)
        crc_value = Crc_GetReflectionOfInt(crc_value);
#endif
    }
...
/* check if it is table method to calculate */
    ...
    {
        /*PRQA S 3120 ++ */ /*VL_QAC_MagicNum*/
        crc_value = Crc_CalCrc32p4Tab[(crc_value ^ (uint32)Crc_DataPtr[i]) & 0xffuL] ^ (crc_value >> 0x8u);
        /*PRQA S 3120 -- */
    }
    /* check if it is hardware method to calculate */
...
uint64 Crc_CalculateCRC64(const uint8* Crc_DataPtr, uint64 Crc_Length, uint64 Crc_StartValue64, boolean Crc_IsFirstCall)
{
    ...
    if (Crc_IsFirstCall)
    {
        crc_value = CRC_INITVALUE64;
    }
    else
    {
        crc_value = Crc_StartValue64;
        crc_value ^= CRC_XORVALUE64;
#if (CRC_RUNTIME == CRC64_MODE)
        crc_value = Crc_GetReflectionOfLonglong(crc_value);
#endif
    }
...
/* check if it is runtime method to calculate */
    ...
    for (i = 0u; i < Crc_Length; i++)
    {
        ...
        /* PRQA S 2824 ++ */ /* VL_Crc_NullPointer */
        crc_value ^= ((uint64)Crc_GetReflectionOfChar(Crc_DataPtr[i]) << CRC_BIT_LENGTH56);
        /* PRQA S 2824 -- */
        for (j = 0u; j < CRC_BIT_LENGTH8; j++)
        {
            /*check if MSB is 1*/
            if ((crc_value & CRC_MSB_ULL) > 0x0uLL)
            {
                crc_value <<= CRC_BIT_LENGTH; /* PRQA S 2921*/ /* VL_Crc_LeftShift */
                crc_value ^= (uint64)CRC_POLY64;
            }
            else
            {
                crc_value <<= CRC_BIT_LENGTH;
    ...
    crc_value = (uint64)Crc_GetReflectionOfLonglong(crc_value);
/* check if it is table method to calculate */
#elif (CRC_TABLE == CRC64_MODE)
    for (i = 0u; i < Crc_Length; i++)
    {
        /*PRQA S 3120 ++ */ /*VL_QAC_MagicNum*/
        crc_value = Crc_CalCrc64Tab[(crc_value ^ (uint64)Crc_DataPtr[i]) & 0xffuLL] ^ (crc_value >> 0x8u);
        /*PRQA S 3120 -- */
    }
    /* check if it is hardware method to calculate */
 ...
 */
void Crc_GetVersionInfo(Std_VersionInfoType* versionInfo) /*PRQA S 1503*/ /*VL_QAC_NoUsedApi*/
{
...
CRC_LOCAL uint8 Crc_GetReflectionOfChar(uint8 input)
{
    ...
    for (i = 0u; i < 0x08u; i++) /* PRQA S 3120 */ /* VL_QAC_MagicNum */
    {
        result |= (uint8)(((input >> i) & 0x01u) << ((uint8)(0x07u - i))); /* PRQA S 3120 */ /* VL_QAC_MagicNum */
    }
...
CRC_LOCAL uint16 Crc_GetReflectionOfShort(uint16 input)
{
    ...
    for (i = 0u; i < 0x10u; i++) /* PRQA S 3120 */ /* VL_QAC_MagicNum */
    {
        result |= (uint16)(((input >> i) & 0x01u) << ((uint16)(0x0Fu - i))); /* PRQA S 3120 */ /* VL_QAC_MagicNum */
    }
...
CRC_LOCAL uint32 Crc_GetReflectionOfInt(uint32 input)
{
    ...
    for (i = 0u; i < 0x20u; i++) /* PRQA S 3120 */ /* VL_QAC_MagicNum */
    {
        result |= (uint32)(((input >> i) & 0x01u) << ((uint32)(0x1Fu - i))); /* PRQA S 3120 */ /* VL_QAC_MagicNum */
    }
...
CRC_LOCAL uint64 Crc_GetReflectionOfLonglong(uint64 input)
{
    ...
    for (i = 0x0uLL; i < 0x40uLL; i++)
    {
        result |= (uint64)(((input >> i) & 0x001uLL) << ((uint64)(0x3FuLL - i)));
    }
