...
/**
  ...
  \li VL_Crc_Redudant
   ...
   \li VL_Crc_Scope
   Reason: The static var should be stored in a fixed area.Placing it
   inside the function block scope could, depending on the compiler and
   optimization settings, result in the object being placed on the stack
   or being re-initialized on each call, which is undesirable for large,
   read-only tables.
   Risk: No risk.
   Prevention: None.
*/
