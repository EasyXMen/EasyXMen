...
#define EA_TOTAL_HEADER_LENGTH (10u + (uint8)((uint8)sizeof(Eep_AddressType) * 2u))
#else
#define EA_TOTAL_HEADER_LENGTH (10u + (uint8)sizeof(Eep_AddressType))
#endif

#define EA_HEADER_ALIGNMENT_LENGTH \
    ((uint16)(((EA_TOTAL_HEADER_LENGTH + (EA_VIRTUAL_PAGE_SIZE - 1U)) / EA_VIRTUAL_PAGE_SIZE) * EA_VIRTUAL_PAGE_SIZE))
...
#define EA_DESERIALIZE(pDeserialPtr, ParamVal, ParamType)                              \
    do                                                                                 \
    {                                                                                  \
        ParamVal = pDeserialPtr[0];                                                    \
        for (uint8 i = 1u; i < sizeof(ParamType); i++)                                 \
        {                                                                              \
            ParamVal = ParamVal | (ParamType)((ParamType)pDeserialPtr[i] << (i * 8u)); \
        }                                                                              \
    } while (0);
...
FUNC(void, EA_PUBLIC_CODE) Ea_Init(P2CONST(Ea_ConfigType, EA_VAR, EA_CONST) ConfigPtr)
{
...
#endif /* EA_DEV_ERROR_DETECT == STD_ON */
    {
        ...
        for (iloop = 0; iloop < Ea_Config.EaBlockCfg->BlockNum; iloop++)
        {
...
FUNC(void, EA_PUBLIC_CODE) Ea_SetMode(MemIf_ModeType Mode)
{
...
#endif /* EA_DEV_ERROR_DETECT == STD_ON */
    {
        for (iloop = 0; iloop < Ea_Config.EaDeviceNum; iloop++)
        {
            ...
            if (NULL_PTR != Ea_Config.EaEepApi[iloop].Eep_SetModeApi)
            {
                Ea_Config.EaEepApi[iloop].Eep_SetModeApi(Mode);
            }
        }
    }
}
...
Ea_Read(uint16 BlockNumber, uint16 BlockOffset, P2VAR(uint8, EA_VAR, EA_VAR) DataBufferPtr, uint16 Length)
{
    ...
    else if (Ea_Config.EaBlockCfg->BlockCfg[blockIndex].EaBlockSize < (BlockOffset + EA_BLOCK_OVER_HEAD))
    {
        Ea_DetReport(EA_SID_READ, EA_E_INVALID_BLOCK_OFS);
    }
    else if (Ea_Config.EaBlockCfg->BlockCfg[blockIndex].EaBlockSize < (Length + BlockOffset + EA_BLOCK_OVER_HEAD))
    {
...
Ea_Write(uint16 BlockNumber, P2CONST(uint8, EA_VAR, EA_CONST) DataBufferPtr)
{
    ...
    else if (NULL_PTR == DataBufferPtr)
    {
...
#endif /* EA_DEV_ERROR_DETECT == STD_ON */
        if ((0u == Ea_Config.EaBlockCfg->BlockCfg[blockIndex].EaNumberOfWriteCycles)
            || (Ea_Config.EaBlockCfg->BlockCfg[blockIndex].EaNumberOfWriteCycles
                > Ea_BlockInfo[blockIndex].BlockWriteCycle))
        {
            ...
            Ea_RunTimeData.EaPendReq.BlockNumber        = BlockNumber;
            Ea_RunTimeData.EaPendReq.BlockIndex         = blockIndex;
            Ea_RunTimeData.EaPendReq.Length             = Ea_Config.EaBlockCfg->BlockCfg[blockIndex].EaBlockSize;
...
FUNC(Std_ReturnType, EA_PUBLIC_CODE) Ea_EraseImmediateBlock(uint16 BlockNumber)
{
    ...
    else if (FALSE == Ea_Config.EaBlockCfg->BlockCfg[blockIndex].EaImmediateData)
    {
        Ea_DetReport(EA_SID_ERASEIMMEDIATE_BLOCK, EA_E_INVALID_BLOCK_NO);
    }
    else
...
static FUNC(uint16, EA_PUBLIC_CODE) Ea_GetBlockIndex(uint16 BlockNumber)
{
    ...
    if ((0u != BlockNumber) && (EA_BLOCKNUM_INVALID != BlockNumber))
    {
        for (iloop = 0; iloop < Ea_Config.EaBlockCfg->BlockNum; iloop++)
        {
            if (BlockNumber == Ea_Config.EaBlockCfg->BlockCfg[iloop].EaBlockNumber)
            {
                ret = iloop;
                break;
            }
...
{
    uint16              blockIndex    = Ea_RunTimeData.EaPendReq.BlockIndex;
    uint8               EaDeviceIndex = Ea_Config.EaBlockCfg->BlockCfg[blockIndex].EaDeviceIndex;
    ...
    else
    {
        ...
        {
            if (MEMIF_IDLE == Ea_Config.EaEepApi[EaDeviceIndex].Eep_GetStatusApi())
            {
                ret = Ea_Config.EaEepApi[EaDeviceIndex].Eep_GetJobResultApi();
                switch (ret)
                {
                case MEMIF_JOB_OK:
...
static FUNC(void, EA_PUBLIC_CODE) Ea_InterWrite(void)
{
    ...
    if (EA_EEP_JOB_IDLE == Ea_RunTimeData.EaEepState)
    {
        ...
        else if (EA_SUB_JOB_INVALID_HEADER_OK == Ea_RunTimeData.EaSubJobStatus)
        {
            Ea_RunTimeData.EaPendReq.DealOffset = EA_HEADER_ALIGNMENT_LENGTH;
            Ea_RunTimeData.EaPendReq.DealSize =
                Ea_Config.EaBlockCfg->BlockCfg[blockIndex].EaBlockSize - EA_HEADER_ALIGNMENT_LENGTH;
...
static FUNC(void, EA_PUBLIC_CODE) Ea_InterErase(void)
{
    ...
    if (EA_EEP_JOB_IDLE == Ea_RunTimeData.EaEepState)
    {
        Ea_RunTimeData.EaPendReq.DealSize = Ea_Config.EaBlockCfg->BlockCfg[blockIndex].EaBlockSize;
        Ea_RunTimeData.EaEepState         = EA_EEP_JOB_ASYNC_READY;
    }
...
static FUNC(void, EA_PUBLIC_CODE) Ea_InterCheckReadHeader(void)
{
    ...
    uint16          blockIndex          = Ea_RunTimeData.EaPendReq.BlockIndex;
    uint16          EaBlockNumber       = Ea_Config.EaBlockCfg->BlockCfg[blockIndex].EaBlockNumber;
    uint16          EaBlockSize         = Ea_Config.EaBlockCfg->BlockCfg[blockIndex].EaBlockSize;
    Eep_AddressType EaBlockStartAddress = Ea_Config.EaBlockCfg->BlockCfg[blockIndex].BlockStartAddress;
    ...
    src = &src[sizeof(uint16)];
    EA_DESERIALIZE((src), (BlockSize), uint16)
    src = &src[sizeof(uint16)];
    EA_DESERIALIZE((src), (BlockWriteCycle), uint32)
    src = &src[sizeof(uint32)];
    EA_DESERIALIZE((src), (BlockValid), uint16)
    src = &src[sizeof(uint16)];
    EA_DESERIALIZE((src), (BlockStartAddress), Eep_AddressType)

    if (EA_BLOCK_INVALID == BlockValid)
    {
        Ea_JobFinish(MEMIF_BLOCK_INVALID);
    }
    else if ((EaBlockStartAddress != BlockStartAddress) || (EaBlockNumber != BlockNumber) || (EaBlockSize != BlockSize))
    {
        Ea_JobFinish(MEMIF_BLOCK_INCONSISTENT);
    }
...
static FUNC(void, EA_PUBLIC_CODE) Ea_InterPrepareWriteHeader(uint8 status)
{
    ...
    uint16          BlockNumber       = Ea_Config.EaBlockCfg->BlockCfg[blockIndex].EaBlockNumber;
    uint16          BlockSize         = Ea_Config.EaBlockCfg->BlockCfg[blockIndex].EaBlockSize;
    uint32          BlockWriteCycle   = Ea_BlockInfo[blockIndex].BlockWriteCycle;
    uint16          BlockVaild        = EA_BLOCK_VALID;
    Eep_AddressType BlockStartAddress = Ea_Config.EaBlockCfg->BlockCfg[blockIndex].BlockStartAddress;
...
static FUNC(void, EA_PUBLIC_CODE) Ea_InterCancel(void)
{
    ...
    if (MEMIF_JOB_PENDING == Ea_Config.EaEepApi[EaDeviceIndex].Eep_GetJobResultApi())
    {
        Ea_Config.EaEepApi[EaDeviceIndex].Eep_CancelApi();
    }
    Ea_JobFinish(MEMIF_JOB_CANCELED);
}
...
static FUNC(void, EA_PUBLIC_CODE) Ea_MemAccReq(Ea_RequestJobType Ea_RequestJob)
{
    ...
    Eep_AddressType address       = (Eep_AddressType)(Ea_RunTimeData.EaPendReq.DealOffset
                                                + Ea_Config.EaBlockCfg->BlockCfg[blockIndex].BlockStartAddress);
    uint8           EaDeviceIndex = Ea_Config.EaBlockCfg->BlockCfg[blockIndex].EaDeviceIndex;
        ...
        if ((MEMIF_IDLE == Ea_Config.EaEepApi[EaDeviceIndex].Eep_GetStatusApi()))
        {
            ...
            case EA_JOB_READ:
                if ((Std_ReturnType)E_OK
                    == Ea_Config.EaEepApi[EaDeviceIndex].Eep_ReadApi(address, DataBuffer, DealLength))
                {
                    Ea_RunTimeData.EaEepState = EA_EEP_JOB_ASYNC_PENDING;
                }
                else
            ...
            case EA_JOB_INVALIDATE:
                if ((Std_ReturnType)E_OK
                    == Ea_Config.EaEepApi[EaDeviceIndex].Eep_WriteApi(address, DataBuffer, DealLength))
                {
                    Ea_RunTimeData.EaEepState = EA_EEP_JOB_ASYNC_PENDING;
                }
                else
                ...
                break;
            case EA_JOB_ERASE:
                if ((Std_ReturnType)E_OK == Ea_Config.EaEepApi[EaDeviceIndex].Eep_EraseApi(address, DealLength))
                {
                    Ea_RunTimeData.EaEepState = EA_EEP_JOB_ASYNC_PENDING;
                }
                else
