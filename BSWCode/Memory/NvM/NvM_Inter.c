...
static FUNC(Std_ReturnType, NVM_CODE) NvM_MultiJobDispatch(void)
{
    ...
    else
    {
        ...
        else if (((uint16)NVM_BLOCK_NUM_ALL < NvM_CurRunning.BlockId) && ((uint8)STD_ON == NvM_MultiJob.ID1WriteMark))
        {
            ...
            NvM_AdminBlock[0].ServiceID = NVM_WRITE_ALL_SERV_ID;

            NvM_GetRamAddress(USELESS_FOR_MULTIJOB);
            NvM_BlockDescriptor[0].NvmRamBlockDataAddress[0] = (uint8)(((uint16)(NVM_COMPILED_CONFIG_ID)) & 0x00ffU);
            NvM_BlockDescriptor[0].NvmRamBlockDataAddress[1] = (uint8)(((uint16)(NVM_COMPILED_CONFIG_ID)) >> 8U);

            NvM_AdminBlock[0].SingleReqResult = NVM_REQ_PENDING;
            NvM_CopyCurBlockInfo();
            l_ReturnValue = E_OK;
        }
        else if ((uint16)NVM_BLOCK_NUM_ALL >= NvM_CurRunning.BlockId)
        {
            /* pending the job */
            NvM_CurRunning.ServiceId                              = NVM_WRITE_ALL_SERV_ID;
