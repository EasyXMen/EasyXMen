...
static FUNC(void, ECUM_CODE) EcuM_HaltSequenceExit(void)
{
...
#endif /*ECUM_MAX_MCU_CORE_NUM > 1*/
    {
        ...
        if ((EcuM_GetPendingWakeupEvents() & ECUM_WKSOURCE_ALARMCLOCK)
            && (EcuMRunData.MasterAlarm != (uint32)0xFFFFFFFF) && (EcuMRunData.MasterAlarm > EcuMRunData.GlobalClock))
        {
            /*Disable all interrupts*/
            DisableAllInterrupts();
            EcuM_GenerateRamHash();
