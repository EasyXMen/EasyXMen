...
/**
  ...
  RuleSorce:puhua_rules-2024.12.rcf
    ...
    \li VL_Det_3218
    Reason:detStatus is designed as a file-scope static structure to centrally control DET debug operations.
            When DET_USE_EXTENDED_DEBUG_SUPPORT is enabled, it is accessed by multiple functions
            (Det_CheckFilterMatch, Det_LogError, etc.). When disabled, these functions are excluded by preprocessor,
            leaving detStatus referenced only in Det_Init(), triggering QAC. This design maintains a single control
            point for debug features while allowing code size optimization.
    Risk: No risk.
    Prevention:None.

    \li VL_Det_3408
    Reason:Det_Config is used to initialize the Det module. In some project configurations, Det_Config may
            not be called during initialization, leaving the variable unused and triggering a QAC warning.
