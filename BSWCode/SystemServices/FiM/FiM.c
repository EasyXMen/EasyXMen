...
FiM_Init(P2CONST(FiM_ConfigType, AUTOMATIC, FIM_APPL_DATA) FiMConfigPtr)
{
...
#endif
    {
        ...
        for (index = 0x0u; index < FIM_NUMBER_OF_FID; index++)
        {
...
#endif
#if (FIM_NUMBER_OF_COMPONENT > 0)
        for (index = 0x0u; index < FIM_NUMBER_OF_COMPONENT; index++)
        {
    ...
    if (TRUE == inhibstu)
    {
        if (0x0u == FiM_FidStu[fidIndex].inhCouter)
        {
            FiM_FidStu[fidIndex].ctrlStu =
                FiM_FidStu[fidIndex].ctrlStu & ((uint8)(~(1u << FIM_FID_PERMISSION_STU_BIT)));
        }
