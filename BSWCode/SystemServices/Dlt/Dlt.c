...
#include "Dlt_MemMap.h"
/* DET application ID */
static CONST(uint32, DLT_CONST) Dlt_Det_ApplId = 0x44455400; /* DET  */
...
#include "Dlt_MemMap.h"
/* DET context ID */
static CONST(uint32, DLT_CONST) Dlt_Det_ContextId = 0x53544400; /* STD  */
#define DLT_STOP_SEC_CONST_32BIT
#include "Dlt_MemMap.h"
...
#endif
    {
        ...
        if ((ret == E_OK) && (RequestResult == NVM_REQ_OK))
        {
...
#endif /* 1u < DLT_CHANNEL_NUM */
            {
                ...
                uint32           logChannelBufferSize = config->LogOutput->pLogChannel[index].DltLogChannelBufferSize;
            ...
            if (Filter == TRUE)
            {
...
#endif /* 1u < DLT_CHANNEL_NUM */
                {
                    ...
                    uint16 logChannelMaxMessageLength =
                        Dlt_ConfigPtr->LogOutput->pLogChannel[ChannelIndex].DltLogChannelMaxMessageLength;
...
#endif /* 1u < DLT_SWC_NUM */
        {
                 ...
                 SwcContextIndex++)
            {
                ...
                if ((appId == SwcApplicationId) && (contextId == SwcContextId))
                {
                    ...
                    contextInfoList->verbose_mode = useVerboseMode;
                    /* Copy the "SessionId" described by the Swc context to the log information */
                    contextInfoList->session_id = SwcSessionId;
                    ...
                    contextInfoList->contextDescription = SwcContextInfoPtr->contextDescription;
...
#endif /* 1u < DLT_CHANNEL_NUM */
        {
            const Dlt_LogChannelType* logChannelPtr = &(Dlt_ConfigPtr->LogOutput->pLogChannel[index]);
...
FUNC(void, DLT_CODE) Dlt_Tx_CopyTxDataToSendBuffer(uint16 ChannelIndex, uint8* Dest, uint32 SendOffset, uint32 Length)
{
    ...
    const Dlt_LogChannelType* logChannelPtr = &(Dlt_ConfigPtr->LogOutput->pLogChannel[ChannelIndex]);
    for (uint32 Cnt = 0; Cnt < Length; Cnt++)
    {
        Dest[Cnt]            = ChannelPtr->buffer[Offset];
...
static FUNC(void, DLT_CODE) Dlt_Tx_Channel(uint16 ChannelIndex)
{
    ...
    PduIdType                 TxPduId              = logChannelPtr->pDltTxPdu->DltTxPduRPduId;
    Dlt_ChannelType*          ChannelPtr           = &(Dlt_Channel[ChannelIndex]);
...
#if (DLT_IF_TRANSMIT == STD_ON)
            TxPduUsesTp = logChannelPtr->pDltTxPdu->DltTxPduUsesTp;
            if (TxPduUsesTp == TRUE)
            {
...
FUNC(void, DLT_CODE) Dlt_Init_From_Cfg(const Dlt_ConfigType* config)
{
    ...
    RunTimePtr->DefaultLogLevel     = config->pLogLevelSetting->DltDefaultLogLevel;
    RunTimePtr->DefaultTraceStatus  = config->pTraceStatusSetting->DltDefaultTraceStatus;
    SchM_Exit_Dlt_SwcContext();
    uint16 index = 0;
#if (1u < DLT_CHANNEL_NUM)
    for (; index < DLT_CHANNEL_NUM; index++)
#endif /* 1u < DLT_CHANNEL_NUM */
    {
        ...
        uint32                  logChannelBufferSize = config->LogOutput->pLogChannel[index].DltLogChannelBufferSize;
        boolean                 logTraceStatusFlag   = config->LogOutput->pLogChannel[index].DltLogTraceStatusFlag;
        Dlt_MessageLogLevelType logChannelThreshold  = config->LogOutput->pLogChannel[index].DltLogChannelThreshold;
        SchM_Enter_Dlt_SwcContext();
...
#endif /* 1u < DLT_SWC_NUM */
    {
        ...
        {
            const Dlt_SwcContextType* SwcContextPtr     = &SwcPtr->pDltSwcContext[iloop];
...
#endif /* 1u < DLT_TRACE_STATUS_ASSIGNMENT_NUM */
            {
                const Dlt_TraceStatusAssignmentTypes* traceStatusAssignmentPtr =
                    &config->pTraceStatusSetting->TraceStatusAssignment[cnt];
...
#endif /* 1u < DLT_LOG_LEVEL_THRESHOLD_NUM */
            {
                const Dlt_LogLevelThesholdType* logLevelThresholdPtr =
                    &config->pLogLevelSetting->LogLevelThreshold[cnt];
                Dlt_ApplicationIDType SwcApplicationId =
...
#endif /* 1u < DLT_LOG_CHANNEL_ASSIGNMENT_NUM */
            {
                const Dlt_LogChannelAssignmentType* logChannelAssignmentPtr =
                    &config->LogOutput->pLogChannelAssignment[cnt];
                Dlt_ApplicationIDType SwcApplicationId =
...
FUNC(uint16, DLT_CODE) Dlt_GetChannelIndexByChannelName(Dlt_LogChannelNameType DltLogChannelId)
{
    ...
    {
#if (DLT_CHANNEL_NUM > 0)
        const Dlt_LogChannelType* logChannelPtr = &Dlt_ConfigPtr->LogOutput->pLogChannel[iloop];
...
FUNC(uint16, DLT_CODE) Dlt_GetChannelIndexByTxPduId(PduIdType id)
{
    uint16 retVal = DLT_CHANNEL_NUM;
    uint16 iloop  = 0;
#if (1u < DLT_CHANNEL_NUM)
...
#if (DLT_CHANNEL_NUM > 0)
        const Dlt_LogChannelType* logChannelPtr = &Dlt_ConfigPtr->LogOutput->pLogChannel[iloop];
        if (logChannelPtr->pDltTxPdu->DltTxHandlePduId == id)
        {
            retVal = iloop;
#if (1u < DLT_CHANNEL_NUM)
            break;
...
FUNC(uint16, DLT_CODE) Dlt_GetSwcSessionIdIndex(Dlt_SessionIDType sessionId) /* PRQA S 1505 */ /* VL_Dlt_1505 */
{
    uint16 index = 0U;
...
#if ((DLT_CHANNEL_NUM > 0) && (DLT_CHANNEL_MAX_BUFFER_LENGTH > 0))
    Dlt_ChannelType* ChannelPtr           = &(Dlt_Channel[ChannelIndex]);
    uint32           logChannelBufferSize = Dlt_ConfigPtr->LogOutput->pLogChannel[ChannelIndex].DltLogChannelBufferSize;
...
FUNC(void, DLT_CODE) Dlt_MoveReadIndex(uint16 ChannelIndex, uint32 length)
{
#if (DLT_CHANNEL_NUM > 0)
    Dlt_ChannelType* ChannelPtr           = &(Dlt_Channel[ChannelIndex]);
    uint32           logChannelBufferSize = Dlt_ConfigPtr->LogOutput->pLogChannel[ChannelIndex].DltLogChannelBufferSize;
...
{
    uint32 EcuId = 0;
    if ((Dlt_ConfigPtr->Protocol != NULL_PTR) && (Dlt_ConfigPtr->Protocol->pDltEcuId != NULL_PTR))
    {
        Dlt_EcuIdCalloutTypes DltEcuIdCallout = Dlt_ConfigPtr->Protocol->pDltEcuId->DltEcuIdCallout;
        ...
        {
            EcuId = Dlt_ConfigPtr->Protocol->pDltEcuId->DltEcuIdValue;
        }
    }
    return EcuId;
}
...
static FUNC(void, DLT_CODE) Dlt_SendOverFlow(uint16 ChannelIndex)
{
    ...
    PduInfoType PduInfo;
    boolean     txPduUsesTp = Dlt_ConfigPtr->LogOutput->pLogChannel[ChannelIndex].pDltTxPdu->DltTxPduUsesTp;
    ...
    PduIdType txPduRPduId = Dlt_ConfigPtr->LogOutput->pLogChannel[ChannelIndex].pDltTxPdu->DltTxPduRPduId;
        ...
        if (ChannelPtr->LastSendStatus == DLT_WAIT_SEND)
        {
            ...
            if (options != 5u)
            {
                ...
                else
                {
                    ...
                    ChannelPtr->SendControlBuffer[offset] = SwcContextInfoPtr->TraceStatus;
                    offset += 1u;
                    /* Check if the option word for the given log message is 7 */
                    if (options == 7u)
                    {
            ...
            const Dlt_LogChannelType* logChannelPtr = &Dlt_ConfigPtr->LogOutput->pLogChannel[ChannelIndex];
            boolean                   txPduUsesTp   = logChannelPtr->pDltTxPdu->DltTxPduUsesTp;
            PduInfoType               PduInfo;
            ...
            PduInfo.SduLength     = Messagelength;
            PduIdType txPduRPduId = logChannelPtr->pDltTxPdu->DltTxPduRPduId;
...
#endif /* 1u < DLT_SWC_NUM */
    {
        ...
        if (CurrentTraceStatus == TRUE)
        {
...
#endif /* 1u < DLT_CHANNEL_NUM */
            {
                ...
                /* Check if the DLT message extension header is support verbose mode */
                boolean VerboseMode = (Dlt_ConfigPtr->Protocol != NULL_PTR)
...
static FUNC(void, DLT_CODE) Dlt_CreateExtendedHeader(const Dlt_CreateExtendedHeaderInfoTypes* CreateExtendedHeader)
{
    ...
    if (BufferDataType == DLT_TRACE_DATA)
    {
        if (NULL_PTR != CreateExtendedHeader->traceInfo)
        {
            ...
            Dlt_MessageTraceType traceInfo = CreateExtendedHeader->traceInfo->traceInfo;
