    ...
    if (pEntry != NULL_PTR)
    {
...
#endif /* DEM_MEM_DEST_MAX_NUM_OF_DTC > 1u */
        {
            ...
            if (MemDest != DEM_MEM_DEST_INVALID)
            {
                ...
                {
                    *BufSize = 0;
                    ret      = Dem_ExtendedDataGetFromEntry(pEntry, RecordNumber, TRUE, DestBuffer, BufSize);
#if (DEM_MEM_DEST_MAX_NUM_OF_DTC > 1u)
                    break;
#endif /* DEM_MEM_DEST_MAX_NUM_OF_DTC > 1u */
                }
...
Dem_GetLampStatusOfJ1939DcmSetDTCFilter(P2VAR(Dem_J1939DcmLampStatusType, AUTOMATIC, DEM_APPL_DATA) LampStatus)
{
    ...
    uint8 milState = DemWIRStatus[DEM_MALFUNCTION_LAMP_INDICATOR];
#endif
#if (DEM_RED_STOP_LAMP_INDICATOR != DemIndicator_INVALID)
    uint8 rslState = DemWIRStatus[DEM_RED_STOP_LAMP_INDICATOR];
#endif
#if (DEM_AMBER_WARING_LAMP_INDICATOR != DemIndicator_INVALID)
    uint8 awlState = DemWIRStatus[DEM_AMBER_WARING_LAMP_INDICATOR];
#endif
#if (DEM_PROTECT_LAMP_INDICATOR != DemIndicator_INVALID)
    uint8 plState = DemWIRStatus[DEM_PROTECT_LAMP_INDICATOR];
#endif
    ...
    P2VAR(Dem_J1939DcmLampStatusType, AUTOMATIC, DEM_APPL_DATA) LampStatus)
{
    ...
    for (iloop = 0; iloop < DemPbCfgPtr->pDemEventParameter[pEvent->IntId].AttrNum; iloop++)
    {
        ...
        if (DemIndicatorRef == DEM_MALFUNCTION_LAMP_INDICATOR)
        {
            ...
            {
                /* SWS_Dem_00883 */
                lampStatus |= (Dem_J1939LampStatus(DemIndicatorBehaviour) << DEM_J1939_LAMP_MIL_OFFSET);
                flashLampStatus |= (Dem_J1939FlashLampStatus(DemIndicatorBehaviour) << DEM_J1939_LAMP_MIL_OFFSET);
            }
            else
            {
                lampStatus |= DEM_J1939_LAMP_MIL_NA;
                flashLampStatus |= DEM_J1939_FLASHLAMP_MIL_NO_FLASH;
            }
        }
        /* Red Stop Lamp */
        if (DemIndicatorRef == DEM_RED_STOP_LAMP_INDICATOR)
        {
            ...
            {
                /* SWS_Dem_00887 */
                lampStatus |= (Dem_J1939LampStatus(DemIndicatorBehaviour) << DEM_J1939_LAMP_RSL_OFFSET);
                flashLampStatus |= (Dem_J1939FlashLampStatus(DemIndicatorBehaviour) << DEM_J1939_LAMP_RSL_OFFSET);
            }
            else
            {
                lampStatus |= DEM_J1939_LAMP_RSL_NA;
                flashLampStatus |= DEM_J1939_FLASHLAMP_RSL_NO_FLASH;
            }
        }
        *LampStatus = ((uint16)lampStatus << 8u) | flashLampStatus;
