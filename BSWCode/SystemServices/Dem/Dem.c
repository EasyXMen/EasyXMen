...
#if ((DEM_ENABLE_SOFT_FILTER_OF_PASS == STD_ON) && (DEM_TRIGGERONPASSEDEANLE == STD_OFF))
        /*soft filter when pass or prepass,if TF/TNCLSC or TNCTOC is flaged then return e_ok whith
         * no deal*/
        else if (
            (EventStatus == DEM_EVENT_STATUS_PASSED) && (DEM_DEBOUNCE_MONITOR_INTERNAL == pEventCfg->AlgorithmType)
 ...
 */
/*************************************************************************/
FUNC(void, DEM_CODE) Dem_MainFunction(void) /* PRQA S 3408 */ /* VL_Dem_3408 */
{
    if (Dem_InitState == DEM_STATE_INIT)
    {
...
#if ((DEM_ENABLE_SOFT_FILTER_OF_PASS == STD_ON) && (DEM_TRIGGERONPASSEDEANLE == STD_OFF))
                        /*soft filter when pass or prepass,if TF/TNCLSC or TNCTOC is flaged then return whith
                         * no deal*/
                        if (((0x00u != DEM_FLAGS_ISSET(pEvent->UdsStatus, DEM_UDS_STATUS_TF))
                             || (0x00u != DEM_FLAGS_ISSET(pEvent->UdsStatus, DEM_UDS_STATUS_TNCSLC))
                             || (0x00u != DEM_FLAGS_ISSET(pEvent->UdsStatus, DEM_UDS_STATUS_TNCTOC))))
