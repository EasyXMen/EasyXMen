...
LinSM_Init(P2CONST(LinSM_ConfigType, AUTOMATIC, LINSM_CONST_PBCFG) ConfigPtr)
{
#if (STD_ON == LINSM_DEV_ERROR_DETECT)
 ...
 */
/******************************************************************************/
FUNC(Std_ReturnType, LINSM_CODE) LinSM_ScheduleRequest(NetworkHandleType network, LinIf_SchHandleType schedule)
{
        ...
        else if (!LinSM_CheckSchedule(chl, schedule))
        {
...
#endif
            {
                ...
                if ((LINSM_FULL_COM == LinSM_RTPtr->channelComMode)
                    && ((LINSM_EXE_NOTHING == LinSM_RTPtr->runningReq)
                        || ((LINSM_EXE_CHANGE_SCHEDULE == LinSM_RTPtr->runningReq)
                            && (LinSM_CfgPtr->allowOverwritePendingScheduleRequest))))
                {
 ...
 */
/******************************************************************************/
FUNC(Std_ReturnType, LINSM_CODE) LinSM_GetCurrentComMode(NetworkHandleType network, ComM_ModeType* mode)
{
    Std_ReturnType ret;
    /*@req <SWS_LinSM_00125> */
    /* the state LINSM_UNINIT is active ,report error */
...
LinSM_RequestComMode(NetworkHandleType network, ComM_ModeType mode)
{
