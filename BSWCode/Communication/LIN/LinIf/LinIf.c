 ...
 * V2.1.9      20250308   HuRongbo   fix bug CPT-13123
 * V2.1.10      20250707  ZouZhiJia  fix bug CPT-15518,CPT-15120
 * V2.1.11     20250p30   ZouZhijia  fix bug
...
#define LINIF_C_SW_PATCH_VERSION 11U
...
/******************************************************************************/
FUNC(Std_ReturnType, LINIF_CODE)
LinIf_GotoSleep(NetworkHandleType Channel)
{
    NetworkHandleType ch  = LinIf_GetLinIfChannel(Channel);
    Std_ReturnType    ret = E_NOT_OK;
...
/******************************************************************************/
FUNC(Std_ReturnType, LINIF_CODE)
LinIf_Wakeup(NetworkHandleType Channel)
{
    NetworkHandleType ch  = LinIf_GetLinIfChannel(Channel);
    Std_ReturnType    ret = E_NOT_OK;
...
/******************************************************************************/
FUNC(Std_ReturnType, LINIF_CODE)
LinIf_GetTrcvMode(NetworkHandleType Channel, P2VAR(LinTrcv_TrcvModeType, AUTOMATIC, LINIF_APPL_DATA) TransceiverModePtr)
{
    Std_ReturnType    ret = E_NOT_OK;
    NetworkHandleType ch  = LinIf_GetLinIfChannel(Channel);
    uint8             linTrcvChannelId;
...
#if (LINIF_LIN_TRCV_WAKEUP_SUPPORT == STD_ON)
            else if (NULL_PTR != chCfgPtr->LinIfTransceiverDrvConfig)
            {
                localWakeupSource = LINIF_GET_LIN_TRCV_WAKEUP_SOURCE(ch);
                if (0u != (localWakeupSource & localWakeupSource))
                {
                    linTrcvChannelId = LINIF_GET_LIN_TRCV_CHANNEL_ID(ch);
...
/******************************************************************************/
FUNC(Std_ReturnType, LINIF_CODE)
LinIf_SetConfiguredNAD(NetworkHandleType Channel, uint8 Nad)
{
    Std_ReturnType    ret = E_NOT_OK;
    NetworkHandleType ch  = LinIf_GetLinIfChannel(Channel);
 ...
 */
/******************************************************************************/
/* PRQA S 2889,6060 ++ */ /* VL_QAC_DerefNullPtr,VL_MTR_LinIf_STM19 */
static FUNC(NetworkHandleType, LINIF_CODE) LinIf_GetLinIfChannelByDriverChId(NetworkHandleType channel)
{
    ...
    LinIf_GetSubstitution(PduIdType TxPduId)
{
    ...
    for (idx = 0u; idx < LINIF_GET_SUB_NUM(); idx++)
    {
 ...
 */
/******************************************************************************/
/* PRQA S 2889,3219,6060 ++ */ /* VL_QAC_DerefNullPtr,VL_LinIf_3219,VL_MTR_LinIf_STM19 */
static FUNC(Std_ReturnType, LINIF_CODE) LinIf_CheckTxPduId(PduIdType TxPduId)
{
    P2CONST(LinIf_TxPduType, AUTOMATIC, LINIF_APPL_CONST) txPduPtr;
    uint16 idx;

    for (idx = 0u; idx < LINIF_GET_TXPDU_NUM(); idx++)
    {
