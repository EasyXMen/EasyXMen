...
static inline LinTp_SlaveRuntimeType* LinTp_GetSlaveRuntimeData(NetworkHandleType tpCh)
{
    return &LinTp_SlaveRTData[tpCh - LINTP_MASTER_CHANNEL_NUMBER];
}
...
FUNC(void, LINIF_CODE) LinTp_SlaveInit(void)
{
    ...
    while (idx > 0u)
    {
        ...
        tpSlaveRTPtr++; /* PRQA S 2983 */ /* VL_LinTp_PointerCalu */
        idx--;
    }
}
/******************************************************************************/
...
LinTp_SlaveMRFIndication(NetworkHandleType ch, P2CONST(uint8, AUTOMATIC, LINIF_APPL_DATA) Lin_SduPtr)
{
    ...
    {
        commCh                               = LINIF_GET_COMM_NETWORK(ch);
        tpCh                                 = LinTp_SlaveGetLinTpChannel(commCh);
        LinTp_SlaveRuntimeType* tpSlaveRTPtr = LinTp_GetSlaveRuntimeData(tpCh);
        if ((LINTP_CHANNEL_BUSY == tpSlaveRTPtr->channelState)
    ...
    P2CONST(PduInfoType, AUTOMATIC, LINIF_APPL_CONST) LinTpTxInfoPtr)
{
    ...
    P2VAR(LinTp_SlaveRuntimeType, AUTOMATIC, LINIF_VAR)
    tpSlaveRTPtr       = LinTp_GetSlaveRuntimeData(tpCh);
...
LinTp_SlaveCheckFunctionAddressFlag(NetworkHandleType ch)
{
    ...
    const LinTp_SlaveRuntimeType* tpSlaveRTPtr = LinTp_GetSlaveRuntimeData(tpCh);
...
static FUNC(void, LINIF_CODE) LinTp_SlaveFFRxMainHandle(NetworkHandleType tpCh)
{
    ...
    if ((BUFREQ_OK == bufRslt) && (tpSlaveRTPtr->UpperBufRemaining >= LINTP_FF_DATA_LEN))
    {
        ...
        if (BUFREQ_OK == bufRslt)
        {
            ...
            nextCFLen = (PduLengthType)LinIf_Min(tpSlaveRTPtr->SduRemaining, LINTP_CF_DATA_LEN_MAX);
...
static FUNC(void, LINIF_CODE) LinTp_SlaveResetRtData(P2VAR(LinTp_SlaveRuntimeType, AUTOMATIC, LINIF_VAR) tpSlaveRTPtr)
{
    ...
    (void)ILib_memset(tpSlaveRTPtr->SduBuf, 0u, LINTP_FRAME_LEN_MAX);
}
