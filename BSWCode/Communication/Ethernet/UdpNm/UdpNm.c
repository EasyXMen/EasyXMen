...
#define UDPNM_C_SW_PATCH_VERSION 18u
...
static FUNC(void, UDPNM_CODE) UdpNm_RxCarWakeUpHadle(uint8 chIndex, const uint8* rxDataPtr);
...
static FUNC(void, UDPNM_CODE)
    UdpNm_StateChange(UdpNm_ChannelIndexType chIndex, Nm_StateType nmOldState, Nm_StateType nmNewState);

static FUNC(void, UDPNM_CODE)
    UdpNm_EntryRepeatMessageState(UdpNm_ChannelIndexType chIndex, Nm_StateType nmOldState, boolean isNetWorkRequest);
...
static FUNC(void, UDPNM_CODE) UdpNm_RxPnEiraEraHandle(uint8 chIndex, const uint8* rxPnDtaPtr);
#endif
...
static FUNC(void, UDPNM_CODE) UdpNm_StateManagement(uint8 chIndex, const UdpNm_EventType* eventFlags);

static FUNC(boolean, UDPNM_CODE) UdpNm_WakeUpSignalHandle(UdpNm_ChannelIndexType chIndex, boolean rxIndication);

static FUNC(void, UDPNM_CODE) UdpNm_BusSleepStateHandle(uint8 chIndex, const UdpNm_EventType* eventFlags);

static FUNC(void, UDPNM_CODE) UdpNm_PrepareBusSleepStateHandle(uint8 chIndex, const UdpNm_EventType* eventFlags);

static FUNC(void, UDPNM_CODE) UdpNm_RepeatMessageStateHandle(uint8 chIndex);

#if UDPNM_NODE_DETECTION_ENABLED == STD_ON
static FUNC(boolean, UDPNM_CODE) UdpNm_NodeDetectStateHandle(UdpNm_ChannelIndexType chIndex, boolean rxRMRFlag);
#endif

static FUNC(void, UDPNM_CODE) UdpNm_NormalOperationStateHandle(uint8 chIndex, const UdpNm_EventType* eventFlags);

static FUNC(void, UDPNM_CODE) UdpNm_ReadySleepStateHandle(uint8 chIndex, const UdpNm_EventType* eventFlags);

static FUNC(void, UDPNM_CODE) UdpNm_RxDataMainHandle(uint8 chIndex, UdpNm_EventType* eventFlags);
...
static FUNC(void, UDPNM_CODE) UdpNm_EnterBusSleepModeHandle(UdpNm_ChannelIndexType chIndex, Nm_StateType nmOldState);
...
Std_ReturnType UdpNm_NetworkRelease(NetworkHandleType nmChannelHandle)
{
...
#endif
    {
        ...
        {
            UdpNm_EnterBusSleepModeHandle(chIndex, chRTPtr->udpNmState);
        }
...
/* clang-format on */
{
...
#endif
    {
        ...
        (void)ILib_memcpy(&chRTPtr->txPduData[userDataOffset], nmUserDataPtr, userDataLength);
        SchM_Exit_UdpNm_Context();
...
Std_ReturnType UdpNm_Transmit(PduIdType TxPduId, const PduInfoType* PduInfoPtr)
{
...
#endif
    {
        ...
        for (chIndex = 0; chIndex < UDPNM_NUMBER_OF_CHANNEL; chIndex++)
        {
...
/* @req SWS_UdpNm_00337, @req SWS_UdpNm_00339*/
...
#endif
            {
                ...
                {
                    uint8 rawCbvData = chRTPtr->rxPduData[chCfgPtr->PduCbvPosition];
                    if ((rawCbvData & UDPNM_CBV_BIT_RMR_MASK) > 0u)
                    {
                ...
                UdpNm_RxCarWakeUpHadle(chIndex, PduInfoPtr->SduDataPtr);
#endif
/* @req SWS_UdpNm_00037 */
        ...
        if (PduInfoPtr->SduLength >= chCfgPtr->UdpNmPduLength)
        {
            ...
            if (chIndex < UDPNM_NUMBER_OF_CHANNEL)
            {
                ...
                if (chCfgPtr->UserDataTxPdu != NULL_PTR)
                {
                    ...
                    {
                        SchM_Enter_UdpNm_Context();
                        /* If 'chRTPtr' is declared with the 'const' qualifier, it will trigger the QAC advisory
                         * "Function argument points to a more heavily qualified type." This behavior is reasonable. */
                        (void)ILib_memcpy(
...
#endif
                PduInfoPtr->SduLength = chCfgPtr->UdpNmPduLength;
                /* @req SWS_UdpNm_00378 */
                SchM_Enter_UdpNm_Context();
            ...
            UDPNM_UNUSED_PARAMETER(chCfgPtr);
        }
    }
...
void UdpNm_MainFunction(NetworkHandleType channel)
{
    if ((UdpNm_InitStatus == UDPNM_INIT) && (channel < UDPNM_NUMBER_OF_CHANNEL))
    {
...
#endif
        {
            ...
            UdpNm_EventType         eventFlags = {0};
            if (chRTPtr->rxPduFlg)
            {
                chRTPtr->rxPduFlg = FALSE;
                UdpNm_RxDataMainHandle(channel, &eventFlags);
            }

            UdpNm_TimerManagement(channel);
            UdpNm_StateManagement(channel, &eventFlags);
#if UDPNM_PASSIVE_MODE_ENABLED == STD_OFF
    ...
    if (chCfgPtr->CarWakeUpRxEnabled && ((carWakeupByte & carWakeupMask) > 0u))
    {
...
#endif
        {
            ...
            if (UDPNM_PDU_OFF != chCfgPtr->PduNidPosition)
            {
                ...
                if (chCfgPtr->CarWakeUpFilterNodeId == rxNodeId)
                {
    ...
    UdpNm_StateChange(UdpNm_ChannelIndexType chIndex, Nm_StateType nmOldState, Nm_StateType nmNewState)
{
    ...
    Nm_StateChangeNotification(UdpNm_CfgPtr->ChannelConfig[chIndex].ComMNetworkHandleRef, nmOldState, nmNewState);
#else
    UDPNM_UNUSED_PARAMETER(nmOldState);
#endif /* STD_ON == UDPNM_STATE_CHANGE_IND_ENABLED */

    SchM_Enter_UdpNm_Context();
    UdpNm_ChRunTime[chIndex].udpNmState = nmNewState;
    SchM_Exit_UdpNm_Context();
}
    ...
    P2CONST(PduInfoType, AUTOMATIC, UDPNM_APPL_DATA) PduInfoPtr)
{
    ...
    if (chCfgPtr->PnEnabled)
    {
        ...
        if (pnRxIndFlg)
        {
            uint8 pnDataLength = UdpNm_CfgPtr->PnInfo->UdpNmPnInfoLength;
            uint8 pnDataOffset = UdpNm_CfgPtr->PnInfo->UdpNmPnInfoOffset;
...
static FUNC(void, UDPNM_CODE) UdpNm_RxPnEiraEraHandle(uint8 chIndex, const uint8* rxPnDtaPtr)
{
    ...
    SchM_Enter_UdpNm_Context();
    UdpNm_InnerChannelType* chRTPtr = &UdpNm_ChRunTime[chIndex]; /* PRQA S 3678 */ /* VL_UdpNm_3678 */
#if UDPNM_PN_ERA_CALC_ENABLED == STD_ON
    const UdpNm_ChannelConfigType* chCfgPtr = &UdpNm_CfgPtr->ChannelConfig[chIndex];
#endif
...
#endif
/* @req SWS_UdpNm_00317 */
#if (STD_ON == UDPNM_COM_USERDATA_SUPPORT) && (STD_OFF == UDPNM_TRIGGER_TRANSMIT_API)
        if (chCfgPtr->UserDataTxPdu != NULL_PTR)
        {
            PduInfoType pduInfo;
            uint8       userData[UDPNM_USER_DATA_MAX_LENGTH] = {0};
...
static FUNC(void, UDPNM_CODE) UdpNm_StateManagement(uint8 chIndex, const UdpNm_EventType* eventFlags)
{
    ...
    case NM_STATE_BUS_SLEEP:
        UdpNm_BusSleepStateHandle(chIndex, eventFlags);
        break;

    case NM_STATE_PREPARE_BUS_SLEEP:
        UdpNm_PrepareBusSleepStateHandle(chIndex, eventFlags);
        break;
    ...
    case NM_STATE_NORMAL_OPERATION:
        UdpNm_NormalOperationStateHandle(chIndex, eventFlags);
        break;

    case NM_STATE_READY_SLEEP:
        UdpNm_ReadySleepStateHandle(chIndex, eventFlags);
        break;
...
static FUNC(boolean, UDPNM_CODE) UdpNm_WakeUpSignalHandle(UdpNm_ChannelIndexType chIndex, boolean rxIndication)
{
    ...
    boolean rxWakeup = (rxIndication && (chRTPtr->udpNmState == NM_STATE_PREPARE_BUS_SLEEP)) ? TRUE : FALSE;
    ...
    /** SWS_UdpNm_00129 SWS_UdpNm_00128
    ...
    {
        UdpNm_EntryRepeatMessageState(chIndex, chRTPtr->udpNmState, isNetWorkRequest);

#if UDPNM_PASSIVE_MODE_ENABLED == STD_OFF
        /**
         *  Only if network request and not passive startup.
         *  */
        if (UdpNm_CfgPtr->ChannelConfig[chIndex].ActiveWakeupBitEnabled && ((isNetWorkRequest && !isPassiveStartUp)))
        {
            /** SWS_UdpNm_00366 */
...
static FUNC(void, UDPNM_CODE) UdpNm_BusSleepStateHandle(uint8 chIndex, const UdpNm_EventType* eventFlags)
{
    ...
    (void)UdpNm_WakeUpSignalHandle(chIndex, eventFlags->rxIndication);
}
...
{
    UdpNm_InnerChannelType* chRTPtr     = &UdpNm_ChRunTime[chIndex];
    boolean                 statChanged = UdpNm_WakeUpSignalHandle(chIndex, eventFlags->rxIndication);
...
static FUNC(void, UDPNM_CODE) UdpNm_RepeatMessageStateHandle(uint8 chIndex)
{
    ...
    if (UdpNm_TestAndClear(&chRTPtr->netRequestFlg))
    {
        ...
        {
            UdpNm_EntryRepeatMessageState(chIndex, chRTPtr->udpNmState, TRUE);
            goto exit; /* PRQA S 2001 */ /* VL_UdpNm_2010 */
        }
    ...
    if (chRTPtr->stateToutFlg)
    {
        ...
        if (UDPNM_NETWORK_REQUESTED == chRTPtr->netRequestStatus)
        {
             ...
             * shall be entered.
             */
            UdpNm_StateChange(chIndex, chRTPtr->udpNmState, NM_STATE_NORMAL_OPERATION);
        ...
        else
        {
             ...
             * shall be entered
             */
            UdpNm_StateChange(chIndex, chRTPtr->udpNmState, NM_STATE_READY_SLEEP);
 ...
 */
        ...
        if (UdpNm_CfgPtr->ChannelConfig[chIndex].NodeDetectionEnabled)
        {
            UdpNm_ClrTxPduCbvBit(chIndex, UDPNM_CBV_BIT_RMR_MASK);
        }
#endif
    }
...
static FUNC(boolean, UDPNM_CODE) UdpNm_NodeDetectStateHandle(UdpNm_ChannelIndexType chIndex, boolean rxRMRFlag)
{
    ...
    const UdpNm_ChannelConfigType* chCfgPtr     = &UdpNm_CfgPtr->ChannelConfig[chIndex];
    UdpNm_InnerChannelType*        chRTPtr      = &UdpNm_ChRunTime[chIndex];
    boolean                        inReqFlag    = UdpNm_TestAndClear(&chRTPtr->repeatMessageRequest);

    if (inReqFlag || rxRMRFlag)
    {
        if (inReqFlag)
        {
            /* @req SWS_UdpNm_00121 SWS_UdpNm_00113 */
            UdpNm_SetTxPduCbvBit(chIndex, UDPNM_CBV_BIT_RMR_MASK);
        }
...
static FUNC(void, UDPNM_CODE) UdpNm_NormalOperationStateHandle(uint8 chIndex, const UdpNm_EventType* eventFlags)
{
    ...
    {
        /* @req SWS_UdpNm_00120 SWS_UdpNm_00119 */
        if (UdpNm_NodeDetectStateHandle(chIndex, eventFlags->repeateMessageIndication))
        {
            UdpNm_EntryRepeatMessageState(chIndex, chRTPtr->udpNmState, FALSE);
        }
    }
...
#if UDPNM_NODE_DETECTION_ENABLED == STD_OFF
    UDPNM_UNUSED_PARAMETER(eventFlags);
#endif
}
...
static FUNC(void, UDPNM_CODE) UdpNm_ReadySleepStateHandle(uint8 chIndex, const UdpNm_EventType* eventFlags)
{
    ...
    if (UdpNm_TestAndClear(&chRTPtr->netRequestFlg))
    {
...
#endif
        {
            ...
            chRTPtr->udpNmState = NM_STATE_NORMAL_OPERATION;
 ...
 */
...
#if (STD_ON == UDPNM_NODE_DETECTION_ENABLED)
    if (chCfgPtr->NodeDetectionEnabled && (chRTPtr->udpNmState == NM_STATE_READY_SLEEP))
    {
        if (UdpNm_NodeDetectStateHandle(chIndex, eventFlags->repeateMessageIndication))
        {
            chRTPtr->udpNmState = NM_STATE_REPEAT_MESSAGE;
    ...
    if ((chRTPtr->udpNmState == NM_STATE_READY_SLEEP) && (chRTPtr->nmTimeoutTimer == 0u))
    {
    ...
    {
    case NM_STATE_REPEAT_MESSAGE:
        UdpNm_EntryRepeatMessageState(chIndex, NM_STATE_READY_SLEEP, netRequestFlg);
        break;

    case NM_STATE_NORMAL_OPERATION:
        UdpNm_StateChange(chIndex, NM_STATE_READY_SLEEP, chRTPtr->udpNmState);
        break;

    case NM_STATE_PREPARE_BUS_SLEEP:
        UdpNm_EnterBusSleepModeHandle(chIndex, NM_STATE_READY_SLEEP);
        break;
...
/* Detection of Remote Sleep Indication Handle */
...
#if (STD_ON == UDPNM_REPEAT_MSG_IND_ENABLED)
        if (UdpNm_CfgPtr->RepeatMsgIndEnabled)
        {
    ...
    if (chCfgPtr->PnEnabled)
    {
        uint8 rxPnData[UDPNM_PN_INFO_LENGTH] = {0};
        SchM_Enter_UdpNm_Context();
        (void)ILib_memcpy(rxPnData, chRTPtr->aggRawPnData, UDPNM_PN_INFO_LENGTH);
        (void)ILib_memset(chRTPtr->aggRawPnData, 0u, UDPNM_PN_INFO_LENGTH);
        SchM_Exit_UdpNm_Context();

        /* @req SWS_UdpNm_00345 @req SWS_UdpNm_00358 */
        UdpNm_RxPnEiraEraHandle(chIndex, rxPnData);
    }
#endif
}
...
static FUNC(void, UDPNM_CODE) UdpNm_EnterBusSleepModeHandle(UdpNm_ChannelIndexType chIndex, Nm_StateType nmOldState)
{
    ...
    SchM_Exit_UdpNm_Context();
    UdpNm_StateChange(chIndex, nmOldState, NM_STATE_PREPARE_BUS_SLEEP);
