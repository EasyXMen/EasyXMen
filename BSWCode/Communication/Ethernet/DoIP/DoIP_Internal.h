...
#define DOIP_ROUTING_ACTIVE_REQ_PAY_LEN_WITH_OEMSPEC 11u
...
typedef enum
{
    ...
    DOIP_VEHICLE_ENTRY_STATE_CONNECTED
...
#if (STD_ON == DOIP_DEV_ERROR_DETECT)
static inline void DOIP_DET_REPORT(uint8 ApiId, uint8 ErrorId)
{
...
#endif /*STD_ON == DOIP_DEV_ERROR_DETECT*/
static inline uint16 DOIP_U8_2_U16(const uint8* array)
{
    return (((uint16)(array)[0] << 8u) + (uint16)(array)[1]);
}
static inline boolean DOIP_GENERICHEADER_CHECK(const uint8* dataPtr, PduLengthType SduLength)
{
    uint8   temp          = dataPtr[0] ^ 0xffu;
    uint16  playloadType  = DOIP_U8_2_U16(&dataPtr[2]);
    boolean playloadCheck = FALSE;

    if ((dataPtr[0] == 0xffu)
        && ((playloadType >= DOIP_VEHICLE_INDENT_REQ) && (playloadType <= DOIP_VEHICLE_INDENT_REQ_WITH_VIN))
        && (temp == dataPtr[1]))
    {
        playloadCheck = TRUE;
    }
    ...
    {
        playloadCheck = ((dataPtr[0] == 1u) || (dataPtr[0] == 2u)) && (temp == dataPtr[1])
                        && (SduLength >= DOIP_GENERIC_HEADER_LEN);
    }
    return playloadCheck;
}

static inline uint32 DOIP_U8_2_U32(const uint8* array)
{
    return (
        ((uint32)(array)[0] << 24u) + ((uint32)(array)[1] << 16u) + ((uint32)(array)[2] << 8u) + ((uint32)(array)[3]));
}
/***** mini function *****/
static inline void DoIP_u16_2_u8(uint8* array, uint16 data)
{
    array[0] = (uint8)(data >> 8u);
    array[1] = (uint8)data;
}

static inline void DoIP_u32_2_u8(uint8* array, uint32 data)
{
...
/**********************Det function start************************************/
#if (STD_ON == DOIP_DEV_ERROR_DETECT)
static inline FUNC(BufReq_ReturnType, DOIP_CODE) DoIP_SoAdTpCopyTxData_DetCheck(
...
static inline FUNC(BufReq_ReturnType, DOIP_CODE) DoIP_SoAdTpCopyRxData_DetCheck(
    VAR(PduIdType, AUTOMATIC) id,
    P2CONST(PduInfoType, AUTOMATIC, DOIP_CONST) info,
...
static inline FUNC(boolean, DOIP_CODE) DoIP_SoAdIfRxIndication_DetCheck(
...
static inline FUNC(BufReq_ReturnType, DOIP_CODE) DoIP_SoAdTpStartOfReception_DetCheck(
    VAR(PduIdType, AUTOMATIC) id,
...
static inline FUNC(Std_ReturnType, DOIP_CODE) DoIP_TpTransmit_DetCheck(
...
static inline FUNC(Std_ReturnType, DOIP_CODE) DoIP_TriggerTpTransmit(
...
static inline FUNC(Std_ReturnType, DOIP_CODE) DoIP_GetTcpConnIdxBySa(uint16 sa, uint8* index)
{
    uint8 ret = E_NOT_OK;
    uint8 i;
...
#if (DOIP_ROLE_SERVER_SUPPORT == TRUE)
static inline FUNC(void, DOIP_CODE) DoIP_ResetPendingTxIfQueue(TxPendingIfQueueType* Queue)
{
    uint8 i;
...
static inline FUNC(void, DOIP_CODE) DoIP_ResetPendingTxTpQueue(TxPendingTpQueueType* Queue)
{
    uint8 i;
    Queue->pendingPdurPduId = DOIP_INVALID_UINT16;
...
static inline FUNC(Std_ReturnType, DOIP_CODE) DoIP_PduRRxIdExtract(
    ...
    DoIP_PendingTpDequeue(P2VAR(PendingTxQueueTpMsgType, AUTOMATIC, DOIP_VAR) queueMsg)
{
    ...
    if ((queueMsg->tail != queueMsg->head) || (queueMsg->count > 0u)) /*queueMsg not empty*/
    {
        ...
        queueMsg->head++;
        if (queueMsg->head >= DOIP_TP_QUEUE_BUFFER_NUM)
        {
            queueMsg->head = 0u;
        }
    ...
    DoIP_PendingIfDequeue(P2VAR(PendingTxQueueIfMsgType, AUTOMATIC, DOIP_VAR) queueMsg)
{
    ...
    if ((queueMsg->tail != queueMsg->head) || (queueMsg->count > 0u)) /*queueMsg not empty*/
    {
        tempQueue = &queueMsg->pendingQueue[queueMsg->head];
        queueMsg->head++;
        if (queueMsg->head >= DOIP_IF_QUEUE_BUFFER_NUM)
        {
            queueMsg->head = 0u;
        }
        if (queueMsg->count > 0u)
...
static inline FUNC(void, DOIP_CODE) DoIP_IfTpTxConfirmation(PduIdType TxId, DoIP_PduTypeType DoIP_PduType)
{
    PduIdType      PduRTxPdu;
    Std_ReturnType ret = DoIP_GetPduRTxPduRefByTxId(TxId, &PduRTxPdu, DoIP_PduType);
    if (DoIP_PduType == DOIP_TPPDU)
    {
        PduR_DoIPTpTxConfirmation(PduRTxPdu, E_NOT_OK);
    }
    else if (DoIP_PduType == DOIP_IFPDU)
    {
        PduR_DoIPIfTxConfirmation(PduRTxPdu);
    }
    else
    {
        /*idle qac */
