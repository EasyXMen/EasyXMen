...
/* PRQA S 6520 EOF */ /* VL_MTR_DoIP_STVAR */
/*******************************************************************************
**                      Include Section                                       **
*******************************************************************************/
...
VAR(PendingRoutingActiveType, DOIP_VAR_CLEARED) DoIP_PendingRoutMsg;
#define DOIP_STOP_SEC_VAR_CLEARED_UNSPECIFIED
#include "DoIP_MemMap.h"
...
VAR(boolean, DOIP_VAR_INIT) DoIP_GidSynStatus = FALSE; /* PRQA S 1533 */ /* VL_QAC_OneRefSymbol */
#define DOIP_STOP_SEC_VAR_INIT_BOOLEAN
#include "DoIP_MemMap.h"
...
DOIP_LOCAL FUNC(Std_ReturnType, DOIP_CODE) DoIP_PendingNonDiagUnreadqueue(
    PendingTxQueueNonDiagMsgType* queueMsg,
    PduIdType                     soAdTxPduId,
    const PduInfoType*            info);
DOIP_LOCAL FUNC(Std_ReturnType, DOIP_CODE) DoIP_PendingTpUnreadqueue(
    PendingTxQueueTpMsgType* queueMsg,
    PduIdType                pduRTxId,
    P2CONST(PduInfoType, AUTOMATIC, DOIP_VAR) pdurTxInfo);
DOIP_LOCAL FUNC(Std_ReturnType, DOIP_CODE) DoIP_PendingIfUnreadqueue(
...
DOIP_LOCAL FUNC(void, DOIP_CODE) DoIP_SendRoutineActiveRsp(
    ...
    P2CONST(uint8, AUTOMATIC, DOIP_VAR) responseData,
...
DOIP_LOCAL FUNC(Std_ReturnType, DOIP_CODE) DoIP_SoAdTxIdExtrac(
    PduIdType DoIPPduRTxId,
    P2CONST(PduInfoType, AUTOMATIC, DOIP_CONST) info,
    P2VAR(PduIdType, AUTOMATIC, DOIP_VAR) soadID);

DOIP_LOCAL FUNC(Std_ReturnType, DOIP_CODE) DoIP_ServerHeaderEvaluateWithPayloadType(
    VAR(uint16, AUTOMATIC) txPduId,
    VAR(uint8, AUTOMATIC) bufIdx,
    VAR(SoAd_SoConIdType, AUTOMATIC) SoConId);

DOIP_LOCAL FUNC(void, DOIP_CODE) DoIP_DiagnosticHeaderFill(
    P2CONST(PduInfoType, AUTOMATIC, DOIP_VAR) info,
    VAR(uint8, AUTOMATIC) bufferId,
    VAR(uint16, AUTOMATIC) sa,
    VAR(uint16, AUTOMATIC) ta);

DOIP_LOCAL FUNC(Std_ReturnType, DOIP_CODE) DoIP_PdurSaTaExtract(
 ...
 */
/***********************************************************************/
/* PRQA S 6040 ++ */ /* VL_MTR_DoIP_STPAR */
DOIP_LOCAL FUNC(BufReq_ReturnType, DOIP_CODE) DoIP_DiagMsgRxToPduR(
    PduIdType          pdurRxId,
    const PduInfoType* info,
 ...
 */
/***********************************************************************/
/* PRQA S 6010, 6020 ++ */ /* VL_MTR_DoIP_STLIN, VL_MTR_DoIP_STCYC */
DOIP_LOCAL FUNC(BufReq_ReturnType, DOIP_CODE) DoIP_DiagMsgHandleHeaderReceived(
    DoIP_RoleType role,
    P2CONST(PduInfoType, AUTOMATIC, DOIP_CONST) info,
    P2VAR(PduLengthType, AUTOMATIC, DOIP_VAR) bufferSizePtr,
    P2CONST(DoIP_DiagRcvHandleType, AUTOMATIC, DOIP_VAR) receiveHandle)
/* PRQA S 6010, 6020 -- */
{
    ...
    { /*copy sa ta to internal buffer*/
        ...
        if (TRUE == saRegested)
        {
            ...
            {
                taIsFound = FALSE;
                for (i = 0u; (i < DoIP_PBCfgPtr->DoIPChannelNbr) && (taIsFound == FALSE); i++)
                {
                ...
                { /* Based on req, payloadDataLength(length-4) is compared with MaxBytes-4, */
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            { /*If it is a gateway channel, add the gateway attribute to the role.*/
                                doipChannel = &DoIP_PBCfgPtr->DoIPChannel[i];
    ...
    for (i = 0u; i < DOIP_MAX_TCP_CONNECTIONS; i++)
    {
        ...
        if ((txCtrl->bufferState == DOIP_BUFFER_IDLE) && (txCtrl->processStatus == DOIP_TX_PROCESS_IDLE))
        {
            ...
            if (NULL_PTR != TpQueue)
            {
                ...
                {
                    if (E_NOT_OK
                        == DoIP_PendingTpUnreadqueue(
                            &pBuffer->pendingTxTpQueue,
                            TpQueue->pendingPdurPduId,
                            &TpQueue->pendingPduInfo))
                    {
                        DoIP_IfTpTxConfirmation(TpQueue->pendingPdurPduId, DOIP_TPPDU);
                    }
                    DoIP_ResetTcpInnerTxBuf(i);
                }
            }
            else
            {
                ...
                if (NULL_PTR != IfQueue)
                {
                    ...
                    {
                        if (E_NOT_OK
                            == DoIP_PendingIfUnreadqueue(
                                &pBuffer->pendingTxIfQueue,
                                IfQueue->pendingPdurPduId,
                                &IfQueue->pendingPduInfo))
                        {
                            DoIP_IfTpTxConfirmation(IfQueue->pendingPdurPduId, DOIP_IFPDU);
                        }
                        DoIP_ResetTcpInnerTxBuf(i);
                    }
                }
                ...
                { /*check non diag pengding message*/
                    ...
                    if (NULL_PTR != NonDiagQueue)
                    {
                        ...
                        {
                            (void)DoIP_PendingNonDiagUnreadqueue(
...
DoIP_SoConModeChgAncNty(VAR(SoAd_SoConIdType, AUTOMATIC) SoConId, VAR(SoAd_SoConModeType, AUTOMATIC) Mode)
{
    ...
    const DoIPConnectionsType*     pDoIPConnections = DoIP_PBCfgPtr->DoIPConnections;
    DoIP_VehicleAncConnStatusType* AncConnStatus;

    for (i = 0u; i < pDoIPConnections->DoIPAllVehicleAnnouncementConnectionNbr; i++)
    {
#if (DOIP_ROLE_CLIENT_SUPPORT == TRUE)
        if (i < pDoIPConnections->DoIPClientVehicleAnnouncementConnectionNbr)
        {
...
FUNC(void, DOIP_CODE) DoIP_CloseTcpConnection(VAR(SoAd_SoConIdType, AUTOMATIC) socketId)
{
    ...
    PduIdType                        rxPduId;
    PduIdType                        pdurTxId;
    const TxPendingTpQueueType*      tempTpQueue;
    const TxPendingIfQueueType*      tempIfQueue;
    const TxPendingNonDiagQueueType* tempnonDiagQueue;
    DoIP_TcpInnerBufferType*         pBuffer;
    ...
    if (E_OK == DoIP_GetTcpConnBySocketId(socketId, &i))
    {
        ...
        if (E_OK == DoIP_GetTcpBufIdxByRxPduId(rxPduId, &i))
        {
            ...
            if (pdurTxId == DOIP_INVALID_UINT16)
            {
                ...
                tempTpQueue = DoIP_PendingTpDequeue(&pBuffer->pendingTxTpQueue);
                while (tempTpQueue != NULL_PTR)
                {
                    (void)DoIP_GetPduRTxPduRefByTxId(tempTpQueue->pendingPdurPduId, &pdurTxId, DOIP_TPPDU);
                    if (pdurTxId != DOIP_INVALID_UINT16)
                    {
...
#if (DOIP_ROLE_SERVER_SUPPORT == TRUE)
                pdurTxId    = DOIP_INVALID_UINT16;
                tempIfQueue = DoIP_PendingIfDequeue(&pBuffer->pendingTxIfQueue);
                while (tempIfQueue != NULL_PTR)
                {
                    (void)DoIP_GetPduRTxPduRefByTxId(tempIfQueue->pendingPdurPduId, &pdurTxId, DOIP_IFPDU);
                    if (pdurTxId != DOIP_INVALID_UINT16)
                    {
                        PduR_DoIPIfTxConfirmation(pdurTxId);
                    }
                    tempIfQueue = DoIP_PendingIfDequeue(&pBuffer->pendingTxIfQueue);
                }
#endif
                tempnonDiagQueue = DoIP_PendingNonDiagDequeue(&pBuffer->pendingTxNonDiagQueue);
                while (tempnonDiagQueue != NULL_PTR)
                {
                    tempnonDiagQueue = DoIP_PendingNonDiagDequeue(&pBuffer->pendingTxNonDiagQueue);
                }
            }
            ...
            {
                if (pBuffer->txCtrl.msgType == DOIP_TX_DIAG_IF_MSG)
                {
                    PduR_DoIPIfTxConfirmation(pdurTxId);
                }
                else if (pBuffer->txCtrl.msgType == DOIP_TX_DIAG_TP_MSG)
                {
                    PduR_DoIPTpTxConfirmation(pdurTxId, E_NOT_OK);
                }
                else
 ...
 */
/***********************************************************************/
DOIP_LOCAL FUNC(void, DOIP_CODE) DoIP_TcpNonDiagMsgHandler(
    DoIP_RoleType role,
    VAR(PduIdType, AUTOMATIC) txPduId,
    VAR(uint8, AUTOMATIC) bufIdx,
    uint16 payloadType)
...
/* PRQA S 6030 -- */
{
    ...
    for (i = 0u; ((i < DoIP_PBCfgPtr->DoIPChannelNbr) && (ProcessOn == TRUE)); i++)
    {
        ...
        if ((NULL_PTR != DoIPPduRTxPdu) && (DoIPPduRTxPdu->DoIPPduRTxPduId == pdurid))
        {
            ...
            if ((MetaDataPtr == NULL_PTR) || (metaDataLength == 0u))
            {
    ...
    P2CONST(PduInfoType, AUTOMATIC, DOIP_VAR) pdurTxInfo)
{
    ...
    SchM_Enter_DoIP_ExclusiveArea();
    if ((queueMsg->tail > queueMsg->head) || (queueMsg->count == 0u))
    {
        pendingQueueNum = queueMsg->tail - queueMsg->head;
    }
    else
    {
        pendingQueueNum = (DOIP_TP_QUEUE_BUFFER_NUM - queueMsg->head) + queueMsg->tail;
    }
    if (pendingQueueNum < DOIP_TP_QUEUE_BUFFER_NUM) /*buffer full*/
    {
        pendingQueue                           = &queueMsg->pendingQueue[queueMsg->tail];
        pendingQueue->pendingPdurPduId         = pduRTxId;
        pendingQueue->pendingPduInfo.SduLength = pdurTxInfo->SduLength;
        if (pdurTxInfo->MetaDataPtr != NULL_PTR)
        {
        ...
        {
            pendingQueue->pendingPduInfo.MetaDataPtr = NULL_PTR;
        }

        queueMsg->tail++;
        if (queueMsg->tail >= DOIP_TP_QUEUE_BUFFER_NUM)
        {
            queueMsg->tail = 0;
        }
        if (queueMsg->count < DOIP_TP_QUEUE_BUFFER_NUM)
        {
            queueMsg->count++;
        }
    }
    ...
    P2CONST(PduInfoType, AUTOMATIC, DOIP_VAR) pdurTxInfo)
{
    ...
    if (pendingQueueNum < DOIP_TP_QUEUE_BUFFER_NUM) /*buffer full*/
    {
        if (queueMsg->head == 0u)
        {
            queueMsg->head = DOIP_TP_QUEUE_BUFFER_NUM - 1u;
        }
        else
        {
            queueMsg->head -= 1u;
        }
        pendingQueue                           = &queueMsg->pendingQueue[queueMsg->head];
        pendingQueue->pendingPdurPduId         = pduRTxId;
        pendingQueue->pendingPduInfo.SduLength = pdurTxInfo->SduLength;
        if (pdurTxInfo->MetaDataPtr != NULL_PTR)
        {
    ...
    DoIP_PendingNonDiagEnqueue(PendingTxQueueNonDiagMsgType* queueMsg, PduIdType soAdTxPduId, const PduInfoType* info)
{
    ...
    {
        pendingQueueNum = (DOIP_NON_DIAG_QUEUE_BUFFER_NUM - queueMsg->head) + queueMsg->tail;
    }
    ...
    if (pendingQueueNum < DOIP_NON_DIAG_QUEUE_BUFFER_NUM) /*buffer full*/
    {
        ...
        queueMsg->tail++;
        if (queueMsg->tail >= DOIP_NON_DIAG_QUEUE_BUFFER_NUM)
        {
            queueMsg->tail = 0;
        }
        if (queueMsg->count < DOIP_NON_DIAG_QUEUE_BUFFER_NUM)
        {
            queueMsg->count++;
        }
    }
    ...
    const PduInfoType*            info)
{
    ...
    if (pendingQueueNum < DOIP_NON_DIAG_QUEUE_BUFFER_NUM) /*buffer full*/
    {
        if (queueMsg->head == 0u)
        {
            queueMsg->head = DOIP_NON_DIAG_QUEUE_BUFFER_NUM - 1u;
        }
        else
        {
            queueMsg->head -= 1u;
...
/***********************************************************************/
DOIP_LOCAL_INLINE
FUNC(P2VAR(TxPendingNonDiagQueueType, AUTOMATIC, DOIP_VAR), DOIP_CODE)
DoIP_PendingNonDiagDequeue(P2VAR(PendingTxQueueNonDiagMsgType, AUTOMATIC, DOIP_VAR) queueMsg)
{
    TxPendingNonDiagQueueType* tempQueue = NULL_PTR;

    SchM_Enter_DoIP_ExclusiveArea();
    if ((queueMsg->tail != queueMsg->head) || (queueMsg->count > 0u)) /*queueMsg not empty*/
    {
        tempQueue = &queueMsg->pendingQueue[queueMsg->head];
        queueMsg->head++;
        if (queueMsg->head >= DOIP_NON_DIAG_QUEUE_BUFFER_NUM)
        {
            queueMsg->head = 0u;
        }
        if (queueMsg->count > 0u)
        {
            queueMsg->count--;
        }
    }
...
/*server support apis*/
#if (DOIP_ROLE_SERVER_SUPPORT == TRUE)
/* PRQA S 6010 ++ */ /* VL_MTR_DoIP_STCYC */
DOIP_LOCAL FUNC(void, DOIP_CODE) DoIP_SendVehicleIdentificationRsp(VAR(uint16, AUTOMATIC) txPduId)
/* PRQA S 6010 -- */
{
    uint8                                i, j;
...
/* PRQA S 6040 -- */
{
    ...
    if ((txCtrl->bufferState == DOIP_BUFFER_IDLE) && (txCtrl->processStatus == DOIP_TX_PROCESS_IDLE))
    {
        ...
        if (E_NOT_OK == DoIP_TriggerTpTransmit(txPduId, &pduInfo, txCtrl, DOIP_TX_NON_DIAG_MSG))
        {
            (void)DoIP_PendingNonDiagEnqueue(&pBuffer->pendingTxNonDiagQueue, txPduId, &pduInfo);
            DoIP_ResetTcpInnerTxBuf(bufIdx);
        }
    }
    else
    {
...
DOIP_LOCAL FUNC(void, DOIP_CODE) DoIP_SendAliveCheckReq(VAR(uint16, AUTOMATIC) txPduId, VAR(uint8, AUTOMATIC) bufIdx)
{
    ...
    if ((pBuffer->txCtrl.bufferState == DOIP_BUFFER_IDLE) && (pBuffer->txCtrl.processStatus == DOIP_TX_PROCESS_IDLE))
    {
        DoIP_MemCpy(
            ...
            &pBuffer->txChannel[0],
            pduInfo.SduLength); /*aaa,when dequeue,copy to internal buffer?*/
        if (E_NOT_OK == DoIP_TriggerTpTransmit(txPduId, &pduInfo, &pBuffer->txCtrl, DOIP_TX_NON_DIAG_MSG))
        {
            (void)DoIP_PendingNonDiagEnqueue(&pBuffer->pendingTxNonDiagQueue, txPduId, &pduInfo);
            DoIP_ResetTcpInnerTxBuf(bufIdx);
        }
 ...
 */
/***********************************************************************/
/* PRQA S 6030, 6010 ++ */ /* VL_MTR_DoIP_STMIF, VL_MTR_DoIP_STCYC */
DOIP_LOCAL FUNC(void, DOIP_CODE)
    DoIP_HandleRoutingActiveReq(VAR(PduIdType, AUTOMATIC) txPduId, VAR(uint8, AUTOMATIC) bufIdx)
/* PRQA S 6030, 6010 -- */
{
    ...
    uint8                           routingActivationResponseCode = 0xFFu;
    ...
    P2VAR(uint8, AUTOMATIC, DOIP_VAR) responseCode)
{
     ...
     */
    if (ret == DOIP_SOCKET_ASSIGNMENT_FAILED)
    {
        if ((E_OK == DoIP_GetTcpConnIdxBySa(sa, &i))
            && (DOIP_SOCKET_CONNECTION_REGISTERED == DoIP_TcpConnStatus[i].routActiveState))
        {
            ...
            ret = DOIP_SOCKET_ASSIGNMENT_PENDING;
        }
    }
    /*
        ...
        if (DoIPRoutingActivation->DoIPRoutingActivationNumber == activationType)
        {
            ...
            else
            {
                ...
                Std_ReturnType ret = DoIPRoutingActivation->DoIPRoutingActivationAuthenticationCallback
                                         ->DoIPRoutingActivationAuthenticationFunc(&isAuthen, reqData, resData);
                ...
                else if (ret == 0x10u) /* DOIP_E_PENDING */
                {
                    /* Todo should not simply return true or false, because there is also a pending
                                     status. */
                }
                else
                {
    ...
    for (i = 0u; i < DoIP_PBCfgPtr->DoIPRoutingActivationNbr; i++)
    {
        ...
        if (DoIPRoutingActivation->DoIPRoutingActivationNumber == activationType)
        {
            ...
            else
            {
                ...
                boolean        isAuthen;
                Std_ReturnType ret = DoIPRoutingActivation->DoIPRoutingActivationConfirmationCallback
                                         ->DoIPRoutingActivationConfirmationCallbackFunc(&isAuthen, reqData, resData);
                ...
                {
                    /* Todo should not simply return true or false, because there is also a pending status.
                     */
                }
                else
                {
...
FUNC(void, DOIP_CODE) DoIP_VehicleAncTimeouthandler(void)
{
    ...
    for (i = 0u; i < DOIP_UDP_VEHICLEANNOUNCEMENT_CONNECTIONS; i++)
    {
        ancConnStatus = &DoIP_AncConnStatus[i];
    ...
    if ((E_OK == DoIP_TransmitBufIdExtrac(id, info, &bufferId)) && (E_OK == ret))
    {
        ...
        if (E_OK == ret)
        {
            ...
            if ((E_OK == DoIP_SoAdTxIdExtrac(id, info, &soAdTxPduId))
                && (pBuffer->txCtrl.bufferState == DOIP_BUFFER_IDLE)
    ...
    P2CONST(PduInfoType, AUTOMATIC, DOIP_VAR) pdurTxInfo)
{
    ...
    {
        pendingQueueNum = (DOIP_IF_QUEUE_BUFFER_NUM - queueMsg->head) + queueMsg->tail;
    }
    ...
    else
    {
        ...
        {
            queueMsg->pendingQueue[qID].pendingPduInfo.MetaDataPtr = NULL_PTR;
        }

        queueMsg->tail++;
        if (queueMsg->tail >= DOIP_IF_QUEUE_BUFFER_NUM)
        {
            queueMsg->tail = 0u;
        }
        if (queueMsg->count < DOIP_IF_QUEUE_BUFFER_NUM)
        {
            queueMsg->count++;
        }
    }
    ...
    P2CONST(PduInfoType, AUTOMATIC, DOIP_VAR) pdurTxInfo)
{
    ...
    else
    {
        if (queueMsg->head == 0u)
        {
            queueMsg->head = DOIP_IF_QUEUE_BUFFER_NUM - 1u;
        }
        else
        {
            queueMsg->head -= 1u;
        }
        qID                                                   = (uint8)queueMsg->head;
        queueMsg->pendingQueue[qID].pendingPdurPduId          = pduRTxId;
        queueMsg->pendingQueue[qID].pendingPduInfo.SduLength  = pdurTxInfo->SduLength;
        queueMsg->pendingQueue[qID].pendingPduInfo.SduDataPtr = queueMsg->pendingQueue[qID].pendingPdu.sduDataPtr;
        DoIP_MemCpy(
    ...
    VAR(SoAd_SoConIdType, AUTOMATIC) SoConId)
{
    ...
    const DoIP_TcpInnerBufferType* pBuffer       = &DoIP_TcpInnerBuf[bufIdx];
...
FUNC(void, DOIP_CODE) DoIP_GatewayTxFakeResp(uint8 index, uint8 rspCode)
{
    ...
    {
        DoIP_MemCpy(&txBuffer[0], &pBuffer->txChannel[0], pduInfo.SduLength);
        if (E_NOT_OK == DoIP_TriggerTpTransmit(soadTxPduId, &pduInfo, &pBuffer->txCtrl, DOIP_TX_NON_DIAG_MSG))
        {
