...
static FUNC(void, SOMEIPTP_CODE) SomeIpTp_TxCancelHandle(
    ...
    P2VAR(SomeIpTp_TxPduRTType, AUTOMATIC, SOMEIPTP_APPL_DATA) txPduRtPtr,
    uint8 apiId);

SOMEIPTP_LOCAL_INLINE FUNC(void, SOMEIPTP_CODE)
    SomeIpTp_ResetTxRtData(P2VAR(SomeIpTp_TxPduRTType, AUTOMATIC, SOMEIPTP_APPL_DATA) txPduRtPtr);
...
static FUNC(PduLengthType, SOMEIPTP_CODE)
    SomeIpTp_CalcSizeGetFromUpperLayer(P2CONST(SomeIpTp_TxPduRTType, AUTOMATIC, SOMEIPTP_APPL_DATA) txPduRtPtr);

static FUNC(uint8*, SOMEIPTP_CODE) SomeIpTp_CalcAddrToCopyDataFromUpperLayer(
    P2CONST(SomeIpTp_TxPduRTType, AUTOMATIC, SOMEIPTP_APPL_DATA) txPduRtPtr,
    P2VAR(uint8, AUTOMATIC, SOMEIPTP_APPL_DATA) sduDataPtr);

static FUNC(void, SOMEIPTP_CODE) SomeIpTp_BuildHeader(
    P2CONST(SomeIpTp_TxPduRTType, AUTOMATIC, SOMEIPTP_APPL_DATA) txPduRtPtr,
    P2VAR(uint8, AUTOMATIC, SOMEIPTP_APPL_DATA) headerAddr);
...
static FUNC(void, SOMEIPTP_CODE) SomeIpTp_RxCancelHandle(
    P2VAR(SomeIpTp_RxPduRTType, AUTOMATIC, SOMEIPTP_APPL_DATA) rxPduRtPtr,
    P2CONST(SomeIpTp_RxNSduType, AUTOMATIC, SOMEIPTP_APPL_CONST) rxNSduCfgPtr,
    uint8 apiId,
    uint8 errorId);

SOMEIPTP_LOCAL_INLINE FUNC(void, SOMEIPTP_CODE)
    SomeIpTp_ResetRxRtData(P2VAR(SomeIpTp_RxPduRTType, AUTOMATIC, SOMEIPTP_APPL_DATA) rxPduRtPtr);
...
SomeIpTp_Init(const SomeIpTp_ConfigType* config)
{
#if (STD_ON == SOMEIPTP_DEV_ERROR_DETECT)
    if (NULL_PTR == config)
    {
...
#endif
    {
        ...
        for (pduIdx = 0u; pduIdx < SOMEIPTP_RX_PDU_NUM; pduIdx++)
        {
        ...
        for (pduIdx = 0u; pduIdx < SOMEIPTP_TX_PDU_NUM; pduIdx++)
        {
...
SomeIpTp_Transmit(PduIdType TxPduId, const PduInfoType* PduInfoPtr)
{
...
#endif
    {
        P2VAR(SomeIpTp_TxPduRTType, AUTOMATIC, SOMEIPTP_APPL_DATA)
...
/******************************************************************************/
FUNC(void, SOMEIPTP_CODE)
SomeIpTp_RxIndication(PduIdType RxPduId, const PduInfoType* PduInfoPtr)
{
#if (STD_ON == SOMEIPTP_DEV_ERROR_DETECT)
    boolean detNoErr = TRUE;
    if (SOMEIPTP_UNINIT == SomeIpTp_InitStatus)
...
FUNC(void, SOMEIPTP_CODE) SomeIpTp_MainFunctionTx(void)
{
    ...
    if (SOMEIPTP_INIT == SomeIpTp_InitStatus)
    {
        ...
        for (ch = 0u; ch < SOMEIPTP_CHANNEL_NUM; ch++)
        {
...
static FUNC(void, SOMEIPTP_CODE) SomeIpTp_RxTimerHandle(uint16 ch)
{
    ...
    P2VAR(SomeIpTp_RxPduRTType, AUTOMATIC, SOMEIPTP_APPL_DATA)
    ...
    SomeIpTp_RxMessageCheck(PduIdType RxPduId, P2CONST(PduInfoType, AUTOMATIC, SOMEIPTP_APPL_DATA) PduInfoPtr)
{
    ...
    {
        if (ILib_memcmp(PduInfoPtr->SduDataPtr, (uint8*)&rxPduRtPtr->header, SOMEIPTP_SOMEIP_HEADER_LEN) != 0u)
        {
