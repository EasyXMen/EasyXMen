...
/* PRQA S 0306 EOF */ /* VL_TcpIp_0306 */
/* PRQA S 2982 EOF */ /* VL_TcpIp_2982 */
/* PRQA S 1505 EOF */ /* VL_TcpIp_1505 */
/* PRQA S 0326 EOF */ /* VL_TcpIp_0326 */
/***********************************************************************************************************************
 *  INCLUDES
 ***********************************************************************************************************************/
#include "TcpIp_Cfg.h"
...
#define TCPIP_SOCKID_CAL_ARYADR(ptr) \
    (TcpIp_SocketIdType)(((size_t)(ptr) - (size_t)(TcpIp_SocketTable)) / sizeof(TcpIp_SocketTable[0]))
...
TCPIP_LOCAL
CONST(TcpIp_SocketParaHandleType, TCPIP_VAR)
TcpIp_SocketParaHandleTebla[] = /* PRQA S 3218 */ /* VL_TcpIp_3218 */
    {
...
TCPIP_LOCAL FUNC(uint32, TCPIP_CODE) TcpIp_CloseAllSocket(VAR(uint8, AUTOMATIC) ctrlIdx);
    ...
    VAR(TcpIp_IpAddrStateType, AUTOMATIC) ipAdrState)
{
...
#endif /* defined(TCPIP_CHECK) */
    {
        ...
        for (uint32 index = 0u; index < TcpIp_LCfg.SocketOwnerNum; index++)
...
TCPIP_LOCAL FUNC(uint32, TCPIP_CODE) TcpIp_CloseAllSocket(VAR(uint8, AUTOMATIC) ctrlIdx)
{
    ...
    for (uint32 index = 0; index < TCPIP_SOCKET_NUM; index++)
    {
...
#if (STD_ON == TCPIP_TCP_TLS_ENABLED)
        newSocketMngPtr->listenSocketId = (TCPIP_SOCKID_CAL_ARYADR(socketMngPtr));
#endif
    ...
    TcpIp_CopyTcpMainHandle(P2VAR(TcpIp_SocketHandleType, AUTOMATIC, TCPIP_APPL_VAR) socketMngPtr)
{
    ...
    if (sendFlag > 0u)
    {
        ...
        if ((err_t)ERR_OK != lw_ret)
        {
            ...
            if (NULL_PTR != socketMngPtr->txBuf) /* PRQA S 2992,2996 */ /* VL_TcpIp_2992,VL_TcpIp_2996 */
            {
                SchM_Enter_TcpIp_ExclusiveArea_Tx();
                (void)pbuf_free(socketMngPtr->txBuf);
                SchM_Exit_TcpIp_ExclusiveArea_Tx();
    ...
    VAR(TcpIp_EventType, AUTOMATIC) event)
{
    ...
    TcpIp_SocketIdType      socketId      = (TCPIP_SOCKID_CAL_ARYADR(socketMngPtr));
    ...
    TcpIp_TcpExtargCallbackPcbDestroyed(VAR(u8_t, AUTOMATIC) id, P2VAR(void, AUTOMATIC, TCPIP_APPL_VAR) data)
{
    ...
    if (NULL_PTR != data)
    {
        TcpIp_SocketIdType      socketId     = (TCPIP_SOCKID_CAL_ARYADR(data));
        TcpIp_SocketHandleType* socketMngPtr = &TcpIp_SocketTable[socketId];
 ...
 */
TCPIP_LOCAL FUNC(void, TCPIP_CODE)
    TcpIp_TcpErrCallback(P2VAR(void, AUTOMATIC, TCPIP_APPL_VAR) arg, VAR(err_t, AUTOMATIC) err)
{
...
#endif /* defined(TCPIP_INNER_CHECK) */
    {
        TcpIp_SocketIdType socketId = (TCPIP_SOCKID_CAL_ARYADR(arg));
...
#endif /* STD_ON == TCPIP_TCP_TLS_ENABLED */
            {
                TcpIp_SocketIdType listenSocketId = (TCPIP_SOCKID_CAL_ARYADR(arg));
    ...
    TcpIp_CtrlLinkAllLocalIsRelease(P2CONST(TcpIp_ControllerStatus, AUTOMATIC, TCPIP_VAR) ctrlPtr)
{
    ...
    for (uint32 loop = 0; loop < TCPIP_CTRL_LOCAL_NUMBER; loop++)
    {
...
TcpIp_ControllerInit(VAR(uint32, AUTOMATIC) ctrlCnt, P2CONST(TcpIp_CtrlType, AUTOMATIC, TCPIP_APPL_DATA) ctrlCfgPtr)
{
    ...
    uint32 ctrlPollCnt = (ctrlCnt > TCPIP_CONTROLLER_NUM) ? TCPIP_CONTROLLER_NUM : ctrlCnt;

    for (uint32 index = 0u; index < ctrlPollCnt; index++)
...
#endif /* STD_ON == TCPIP_MUILT_IPADT_PER_CTRL */
        TcpIp_ControllerVar[index].IcmpPcbPtr = NULL_PTR;
 ...
 */
FUNC(TcpIp_StateType, TCPIP_CODE) TcpIp_GetControlState(VAR(uint32, AUTOMATIC) ctrlIndex)
{
...
TcpIp_LocalAdrInit(
    ...
    P2CONST(TcpIp_LocalAddrType, AUTOMATIC, TCPIP_APPL_DATA) localAdrCfgPtr)
{
    uint32 pollCnt = (localAdrNum > TCPIP_LOCAL_ADR_NUM) ? TCPIP_LOCAL_ADR_NUM : localAdrNum;
#if (TCPIP_LOCAL_ADR_NUM > 1u)

    for (uint32 index = 0u; index < pollCnt; index++)
...
TcpIp_InnerRequestComMode(VAR(uint8, AUTOMATIC) ctrlIdx, VAR(TcpIp_StateType, AUTOMATIC) reqState)
{
    ...
    case TCPIP_STATE_ONLINE:
        ...
        else if (TCPIP_STATE_OFFLINE == reqState)
        {
            ...
            uint32 pendCnt = TcpIp_CloseAllSocket(ctrlIdx);
    ...
    P2CONST(void, AUTOMATIC, TCPIP_APPL_CONST) parameterValue)
{
    ...
    uint32 limtLen = sizeof(TcpIp_SocketParaHandleTebla) / sizeof(TcpIp_SocketParaHandleTebla[0]);

    /* this designed shall be reduce QAC STCYC */
    for (uint32 i = 0u; i < limtLen; i++)
    {
    ...
    P2VAR(uint16, AUTOMATIC, TCPIP_APPL_DATA) portPtr)
{
    ...
    if ((((ctrlMngPtr != NULL_PTR) && (TCPIP_STATE_ONLINE == ctrlMngPtr->ctrlStatus))
...
#endif /* defined(TCPIP_INNER_CHECK) */
        {
            udpTxPbuf->payload = (void*)dataPtr; /* PRQA S 0311 */ /* VL_TcpIp_0311 */
            SchM_Enter_TcpIp_ExclusiveArea_Tx();
    ...
    VAR(uint16, AUTOMATIC) totalLength)
{
    ...
    )
    {
        ...
        if ((NULL_PTR != udpTxPbuf) && (totalLength == udpTxPbuf->tot_len) && (udpTxPbuf->len == udpTxPbuf->tot_len))
#endif /* defined(TCPIP_INNER_CHECK) */
        {
            TcpIp_Up_SocketOwnerCopyTxDataType copyFuncPtr = socketMngPtr->ownerCfgPtr->Up_CopyTxDataFuncPtr;
...
TcpIp_InnerRxIndication(
    ...
    VAR(uint16, AUTOMATIC) lenByte)
{
    SchM_Enter_TcpIp_ExclusiveArea_Rx();
    ...
    SchM_Exit_TcpIp_ExclusiveArea_Rx();
#if defined(TCPIP_INNER_CHECK)
...
#endif /* defined(TCPIP_INNER_CHECK) */
    {
        ...
        {
            struct netif* netifPtr = &TcpIp_ControllerVar[ctrlIdx].netifVar;
            p->if_idx              = netif_get_index(netifPtr);
            p->flags |= flag;
    ...
    P2CONST(TcpIp_SocketHandleType, AUTOMATIC, TCPIP_APPL_VAR) socketMngPtr)
{
    ...
    if (NULL_PTR != acceptFunc)
    {
        ...
        remoteAdrInfo.domain = IP_IS_V4_VAL(reIp) ? TCPIP_AF_INET : TCPIP_AF_INET6;
#endif /* TCPIP_SC1 == TCPIP_SCALABILITY_CLASS */
        TcpIp_SocketIdType newAccSckId = (TCPIP_SOCKID_CAL_ARYADR(socketMngPtr));
 ...
 * @return E_OK ok,another is failed
 */
FUNC(Std_ReturnType, TCPIP_CODE) TcpIp_StartDhcp(VAR(uint32, AUTOMATIC) ctrlIndex)
{
 ...
 * @return E_OK ok,another is failed
 */
FUNC(Std_ReturnType, TCPIP_CODE) TcpIp_StopDhcp(VAR(uint32, AUTOMATIC) ctrlIndex)
{
 ...
 * @return E_OK ok,another is failed
 */
FUNC(Std_ReturnType, TCPIP_CODE) TcpIp_StartAutoIp(VAR(uint32, AUTOMATIC) ctrlIndex)
{
    Std_ReturnType ret = E_NOT_OK;
#if defined(TCPIP_INNER_CHECK)
 ...
 * @return E_OK ok,another is failed
 */
FUNC(Std_ReturnType, TCPIP_CODE) TcpIp_StopAutoIp(VAR(uint32, AUTOMATIC) ctrlIndex)
{
    Std_ReturnType ret = E_NOT_OK;
#if defined(TCPIP_INNER_CHECK)
...
#if ((TCPIP_SC1 == TCPIP_SCALABILITY_CLASS) || (TCPIP_SC3 == TCPIP_SCALABILITY_CLASS))
static FUNC(u8_t, TCPIP_CODE)
    TcpIp_IcmpRecvCallback(void* arg, struct raw_pcb* pcb, struct pbuf* p, const ip_addr_t* addr)
{
    u8_t  ret    = 0u;
    u16_t hdrlen = (u16_t)IPH_HL_BYTES((struct ip_hdr*)p->payload);

#if defined(TCPIP_INNER_CHECK)
    /* PRQA S 2995 ++ */ /* VL_TcpIp_2995 */
    /* @ref ip4_input() and icmp_input() */
    if ((hdrlen <= p->len) && (hdrlen <= p->tot_len) && (hdrlen >= (u16_t)IP_HLEN) && (p->len >= (sizeof(u16_t) * 2u)))
    /* PRQA S 2995 -- */ /* VL_TcpIp_2995 */
#endif
    {
        u8_t type;

        (void)pbuf_remove_header(p, hdrlen);

        type = *((u8_t*)p->payload);
        if ((u8_t)ICMP_ECHO != type)
        {
            const TcpIp_IcmpLConfigType*        icmpLCfgPtr = TcpIp_LCfg.Ipv4LCfgPtr->IcmpLCfgPtr;
            const TcpIp_User_IcmpMsgHandlerType icmpMsgFunc = icmpLCfgPtr->User_IcmpMsgHandlerFuncPtr;
            TcpIp_LocalAddrIdType               localAddrId = (TcpIp_LocalAddrIdType)(uint32)arg;
            TcpIp_SockAddrType                  remoteAddr  = {0};
            u8_t                                code = *(((u8_t*)p->payload) + 1); /* PRQA S 0489 */ /* VL_TcpIp_0489 */

            TCPIP_LWIP_TO_LOCAL_IPADDR(remoteAddr.addr, addr->addr);
#if (TCPIP_SC1 == TCPIP_SCALABILITY_CLASS)
            remoteAddr.domain = TCPIP_AF_INET;
#elif (TCPIP_SC2 == TCPIP_SCALABILITY_CLASS)
            remoteAddr.domain = TCPIP_AF_INET6;
#else
            remoteAddr.domain = IP_IS_V4_VAL(addr) ? TCPIP_AF_INET : TCPIP_AF_INET6;
#endif

            p = pbuf_coalesce(p, PBUF_RAW); /* PRQA S 1338 */ /* VL_TcpIp_1338 */
            icmpMsgFunc(localAddrId, &remoteAddr, pcb->ttl, type, code, p->len, (uint8*)p->payload);
            (void)pbuf_free(p);
    ...
    P2CONST(TcpIp_IcmpTransDataType, AUTOMATIC, TCPIP_APPL_VAR) icmpParaPtr)
{
    ...
    if (TCPIP_STATE_ONLINE == ctrlMngPtr->ctrlStatus)
    {
        ...
        if (NULL_PTR == ctrlMngPtr->IcmpPcbPtr)
        {
            ctrlMngPtr->IcmpPcbPtr = raw_new(IP_PROTO_ICMP);
        }
...
#endif
        {
            struct raw_pcb* icmpPcbPtr = ctrlMngPtr->IcmpPcbPtr;

            lwRet = raw_bind(icmpPcbPtr, &TcpIp_LocalAdrTable[ipAdr].ipAdrSrc);
            raw_bind_netif(icmpPcbPtr, &ctrlMngPtr->netifVar);
            if (NULL_PTR != TcpIp_LCfg.Ipv4LCfgPtr)
            {
                const TcpIp_IcmpLConfigType* icmpLCfgPtr = TcpIp_LCfg.Ipv4LCfgPtr->IcmpLCfgPtr;

                if ((NULL_PTR != icmpLCfgPtr) && (NULL_PTR != icmpLCfgPtr->User_IcmpMsgHandlerFuncPtr))
                {
                    raw_recv(icmpPcbPtr, &TcpIp_IcmpRecvCallback, (void*)ipAdr);
                }
            }
...
#endif
            {
...
#endif
                {
                    ...
                    icmphdr->id     = 0u;
                    icmphdr->seqno  = 0u;
                    icmpPcbPtr->ttl = icmpParaPtr->ttl;
                    icmphdr->chksum = 0u;
                    /* copy data to pbuf */
#if defined(TCPIP_INNER_CHECK)
                    lwRet = pbuf_take_at(
                ...
                if ((err_t)ERR_OK == lwRet)
                {
                    ...
                    lwRet = raw_sendto(icmpPcbPtr, q, &rometeIp);
                }
...
{
    for (uint32 index = 0u; index < TCPIP_SOCKET_NUM; index++)
    {
        TcpIp_SocketHandleType* socketMngPtr = &TcpIp_SocketTable[index];
#if (STD_ON == TCPIP_TCP_ENABLED)
...
FUNC(void, TCPIP_CODE) TcpIp_ControlPollMainHandle(void)
{
    ...
    for (uint32 index = 0u; index < TCPIP_CONTROLLER_NUM; index++)
#else /* (1u >= TCPIP_CONTROLLER_NUM */
    uint32 index = 0u;
...
TcpIp_GetControlIpFramePriority(VAR(uint8, AUTOMATIC) ctrlIdx)
{
    uint8 priority;
    ...
    P2CONST(uint8, AUTOMATIC, TCPIP_APPL_DATA) physAddrPtr)
{
    ...
    for (uint32 i = 0; i < ETH_HWADDR_LEN; i++)
    {
...
FUNC(void, TCPIP_CODE) TcpIp_ArpClearAllDynamicEntry(VAR(uint32, AUTOMATIC) ctrlIndex)
{
#if defined(TCPIP_INNER_CHECK)
