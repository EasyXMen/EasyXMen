...
typedef struct
{
    ...
    P2FUNC(Std_ReturnType, ETHIF_APPL_CODE, EthTrcvAPI_TransceiverLinkStateRequest)
    (uint8 TrcvIdx, EthTrcv_LinkStateType LinkState);
#if (ETHIF_ETHTRCV_AUTOSAR_VERSION >= ETHIF_ETHTRCV_AUTOSAR_450)
    P2FUNC(Std_ReturnType, ETHIF_APPL_CODE, EthTrcvAPI_GetTransceiverMode)
    ...
    P2FUNC(Std_ReturnType, ETHIF_APPL_CODE, EthTrcvAPI_GetLinkState)
    (uint8 TrcvIdx, EthTrcv_LinkStateType* LinkStatePtr);
#if (ETHIF_ETHTRCV_AUTOSAR_VERSION >= ETHIF_ETHTRCV_AUTOSAR_450)
    P2FUNC(Std_ReturnType, ETHIF_APPL_CODE, EthTrcvAPI_SetTransceiverMode)
...
{
    uint8                      EthIfTrcvIdx;
    const EthIfEthTrcvRefType* EthIfEthTrcvCfgRef;
} EthIf_TrcvType;
...
typedef struct
{
    ...
    const EthIfEthCtrlRefType* EthIfEthCtrlCfgRef;
...
typedef struct
{
    ...
    EthIf_ULTxConfirmation; /* pointer to hold Tx confirmation functions list */
    P2CONST(EthIfULRxIndicationType, AUTOMATIC, ETHIF_APPL_CONST)
    EthIf_ULRxIndication; /* Ptr to Rx indication function list */
