 ...
 * V2.0.20 [2025/07/21] [fupeng.yu] In the VLAN, resolve the CtrlIdx parameter of called EthSM_CtrlModeIndication error,
 *                                  when EthIf_SetControllerMode was first called and the CtrlIdx > 0 .
 */
...
#define ETHIF_C_SW_PATCH_VERSION 20U
...
#define ETHIF_TRCV_ETHTRCV(TrcvIdx)         EthIf_ConfigPtr->EthIfTrcvCfg[TrcvIdx].EthIfEthTrcvCfgRef->EthIfEthTrcvIdx
#define ETHIF_TRCV_ETHTRCVDRIVERID(TrcvIdx) EthIf_ConfigPtr->EthIfTrcvCfg[TrcvIdx].EthIfEthTrcvCfgRef->EthTrcvDriverId
#define ETHIF_TRCV_WAKEUPSOURCE(TrcvIdx)    EthIf_ConfigPtr->EthIfTrcvCfg[TrcvIdx].EthIfEthTrcvCfgRef->WakeUpSource
...
#define ETHIF_PHYSCONTROLLER_ETHCONTRLLER(PhysIdx) \
    EthIf_ConfigPtr->EthIf_PhysControllerCfg[PhysIdx].EthIfEthCtrlCfgRef->EthIfEthCtrlId
#define ETHIF_PHYSCONTROLLER_ETHDRIVERID(PhysIdx) \
    EthIf_ConfigPtr->EthIf_PhysControllerCfg[PhysIdx].EthIfEthCtrlCfgRef->EthDriverId
...
FUNC(void, ETHIF_CODE) EthIf_Init(P2CONST(EthIf_ConfigType, ETHIF_APPL_DATA, ETHIF_VAR) CfgPtr)
{
...
#endif /* STD_ON == ETHIF_DEV_ERROR_DETECT */
    {
        ...
        {
            EthIf_Internal.EthIfCtrlRunState[index].RequestSetCtrlMode = FALSE;
...
EthIf_GetTransceiverMode(VAR(uint8, ETHIF_VAR) TrcvIdx, P2VAR(Eth_ModeType, ETHIF_APPL_DATA, ETHIF_VAR) TrcvModePtr)
{
...
#endif /*STD_ON == ETHIF_DEV_ERROR_DETECT*/
    {
        ...
        trcvDriverId = ETHIF_TRCV_ETHTRCVDRIVERID(TrcvIdx);
#if (STD_ON == ETHIF_USE_ETHTRCV)
        if (NULL_PTR != EthTrcv_DriverApi[trcvDriverId].EthTrcvAPI_GetTransceiverMode)
        {
#if (ETHIF_ETHTRCV_AUTOSAR_VERSION >= ETHIF_ETHTRCV_AUTOSAR_450)
            ret = EthTrcv_DriverApi[trcvDriverId].EthTrcvAPI_GetTransceiverMode(ethTrcvIdx, TrcvModePtr);
#else
            EthTrcv_ModeType trcvMode;
            ret = EthTrcv_DriverApi[trcvDriverId].EthTrcvAPI_GetTransceiverMode(ethTrcvIdx, &trcvMode);
            if (E_OK == ret)
            {
 ...
 *
 */
/*PRQA S 6040 ++ */ /* VL_MTR_EthIf_QacSTPAR */
            ...
            if (((EthIf_ConfigPtr->EthIfCtrlCfg->EthIfCtrlMtu) >= *LenBytePtr) && (Priority < PRIORITY_MAX))
            {
                *LenBytePtr = (*LenBytePtr + VLAN_TAG_SIZE);
                ...
                if (BUFREQ_OK == ret)
                {
                    ...
                    IntenalBuf[0] &= (uint8)(~((uint8)(1u << 4u)));
                    ...
                    IntenalBuf[2] = (uint8)((uint8)(FrameType >> 8u));
...
#endif /* ETHIF_VLAN_USED == STD_ON */
            if ((E_OK == ret) && (EthIf_ConfigPtr->EthIf_ULTxConfirmation != NULL_PTR))
            {
...
#endif /*STD_ON == ETHIF_DEV_ERROR_DETECT*/
    {
        if (NULL_PTR != EthIf_ConfigPtr->EthIf_ULTxConfirmation)
        {
            for (i = 0; i < EthIf_ConfigPtr->EthIfOwnersCount; i++)
            {
                if (BufIdx == EthIf_Internal.ethIfRunTime[i].bufferIdx)
                {
                    ...
                    if (ret == E_OK)
                    {
                        ...
                        if (NULL_PTR != EthIf_ConfigPtr->EthIf_ULTxConfirmation[handle])
                        {
#if (ETHIF_ETH_AUTOSAR_VERSION < ETHIF_ETH_AUTOSAR_431)
                            Std_ReturnType Result = E_OK;
#endif /* ETHIF_ETH_AUTOSAR_VERSION < ETHIF_ETH_AUTOSAR_431 */
                            EthIf_ConfigPtr->EthIf_ULTxConfirmation[handle](ethifCrlId, BufIdx, Result);
                        }
                    }
                    break;
                }
    ...
    if (E_NOT_OK != ret)
    {
...
#endif /* ETHIF_VLAN_USED != STD_ON */
        {
            for (i = 0u; i < EthIf_ConfigPtr->EthIfOwnersCount; i++)
            {
                ...
                {
                    handle = EthIf_ConfigPtr->EthIfOwnerCfg[i].EthIfOwner;
                    if ((NULL_PTR != EthIf_ConfigPtr->EthIf_ULRxIndication)
                        && (NULL_PTR != EthIf_ConfigPtr->EthIf_ULRxIndication[handle]))
                    {
                        upperLayerFun = EthIf_ConfigPtr->EthIf_ULRxIndication[handle];
                        (*upperLayerFun)(
                            ethifCtrlId,
                            UpperLayerFrameType,
                            IsBroadcast,
...
#endif /* ETHIF_VLAN_USED == STD_ON */
#if (STD_ON == ETHIF_USE_ETHTRCV)
        for (trcvlIdx = 0u; trcvlIdx < EthIf_ConfigPtr->EthIfTrcvCount; trcvlIdx++)
        {
            if (EthIf_Internal.EthIfTrcvRunState[trcvlIdx].EthIfTrcvMode == ETH_MODE_ACTIVE)
            {
                ...
                if (NULL_PTR != EthTrcv_DriverApi[trcvDriverId].EthTrcvAPI_GetLinkState)
                {
                    if (E_OK
                        == EthTrcv_DriverApi[trcvDriverId].EthTrcvAPI_GetLinkState(
...
#if (ETHIF_VLAN_USED == STD_ON)
        /*@req SWS263*/
        for (physCtrlIdx = 0u; physCtrlIdx < EthIf_ConfigPtr->EthIfPhysControllerCount; physCtrlIdx++)
        {
...
#endif /*STD_ON == ETHIF_DEV_ERROR_DETECT*/
    {
        ...
        if (E_OK == ret)
        {
            ...
            if (ETHIF_INVALID_IDX != refTrcvId)
            {
                ...
                if (NULL_PTR != EthTrcv_DriverApi[trcvDriverId].EthTrcvAPI_SetTransceiverMode)
                {
#if (ETHIF_ETHTRCV_AUTOSAR_VERSION >= ETHIF_ETHTRCV_AUTOSAR_450)
                    (void)EthTrcv_DriverApi[trcvDriverId].EthTrcvAPI_SetTransceiverMode(trcvId, CtrlMode);
#else
                ...
                if (NULL_PTR != EthTrcv_DriverApi[trcvDriverId].EthTrcvAPI_TransceiverLinkStateRequest)
                {
                    EthTrcv_LinkStateType trcvLinkState;

                    trcvLinkState = (ETH_MODE_ACTIVE == CtrlMode) ? ETHTRCV_LINK_STATE_ACTIVE : ETHTRCV_LINK_STATE_DOWN;

                    (void)EthTrcv_DriverApi[trcvDriverId].EthTrcvAPI_TransceiverLinkStateRequest(trcvId, trcvLinkState);
                }
            }
#endif /* STD_ON == ETHIF_USE_ETHTRCV */
            ...
            EthIf_Internal.EthIfEthRunState[CtrlIdx].EthIfEthMode            = CtrlMode;
            EthIf_Internal.EthIfCtrlRunState[ethIfCtrlId].RequestSetCtrlMode = FALSE;
...
static Std_ReturnType EthIf_SearchEthifCrlIndex(uint8* EthifCtrlId, uint8 EthCtrlId)
{
    uint8                      index;
    Std_ReturnType             ret = E_NOT_OK;
    const EthIf_CtrlStateType* ethIfCtrlRunStatePtr;

    for (index = 0u; index < EthIf_ConfigPtr->EthIfCtrlCount; index++)
    {
        ethIfCtrlRunStatePtr = &EthIf_Internal.EthIfCtrlRunState[index];
        if ((ETHIF_CONTROLLER_PHYSCONTROLLER(index) == EthCtrlId) && (ethIfCtrlRunStatePtr->RequestSetCtrlMode))
        {
            *EthifCtrlId = index;
            ret          = E_OK;
            break;
...
static Std_ReturnType EthIf_SetCtrlModeActive(uint8 EthIfCtrlIdx, Eth_ModeType CtrlMode, uint8 EthCtrlIdx)
{
    ...
    if (ETH_MODE_ACTIVE == CtrlMode)
    {
        ...
        EthIf_Internal.EthIfCtrlRunState[EthIfCtrlIdx].RequestSetCtrlMode = TRUE;
        ...
        else
        {
            ...
            ethRunStatePtr->EthIfEthModeCnt++;
            EthIf_InnerSaveCtrlMode(EthIfCtrlIdx, CtrlMode);
            EthIf_Internal.EthIfCtrlRunState[EthIfCtrlIdx].RequestSetCtrlMode = FALSE;
            ret                                                               = E_OK;
        }
    }
    return ret;
