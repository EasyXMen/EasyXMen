...
static Std_ReturnType
    EthTSyn_GetIngressTV(uint8 CtrlIdx, uint8 PortId, const uint8* DataPtr, EthTSyn_TimeStampType* TV);
static Std_ReturnType
 ...
 * Return value:    NA
 */
/* PRQA S 6010,6020,6040 ++ */ /* VL_MTR_EthTSyn_Unchanged, VL_MTR_EthTSyn_STLIN, VL_MTR_EthTSyn_QacSTPAR */
    ...
    uint16        LenByte)
{
...
#endif
    {
            ...
            && (port->timedomainid == DataPtr[4]))
        {
            ...
            const EthTSyn_PortConfigType* portCfg            = &PORT_TOCFGPORT(i);
            ...
            case FOLLOW_UP:
            {
                const EthTSyn_GlobalTimeSlaveCfgType* globalTimeSlaveCfgPtr =
                    portCfg->EthTSynPortRole.EthTSynGlobalTimeSlave;
                boolean isPortIdentityEqual = EthTSyn_IsPortIdentityEqual(port->portIdentity, masterPortIdentity);
                if ((port->SyncStatusType == WAITFOR_FOLLOWUP) && isPortIdentityEqual
            ...
            case PDELAY_RESP:
            {
                ...
                boolean      isPortIdentityEqual = EthTSyn_IsPortIdentityEqual(port->portIdentity, masterPortIdentity);
                if (!isValidIngressTime || (SequenceId != port->PdelayReqSequenceId) || !isPortIdentityEqual)
                {
...
#include "EthTSyn_MemMap.h"
/**
 * The implementation of EthTSyn_GetIngressTV.
 * @param[in]  CtrlIdx   Index of the Ethernet controller within the context of the Ethernet Interface
 * @param[in]  PortId    Index of the port within the context of the Ethernet Interface
 * @param[in]  DataPtr   Pointer to the data of the received Ethernet frame
 * @param[out] TV        The ingress time stamp
 * @retval     E_OK      The ingress time stamp was successfully retrieved
 * @retval     E_NOT_OK  The ingress time stamp could not be retrieved
 */
...
/*
 * The implementation of EthTSyn_GetEgressTV.
 * @param[in]  CtrlIdx   Index of the Ethernet controller within the context of the Ethernet Interface
 * @param[in]  PortId    Index of the port within the context of the Ethernet Interface
 * @param[out] TV        The egress time stamp
 * @param[in]  BufIdx    Index of the buffer within the context of the Ethernet Interface
 * @retval     E_OK      The egress time stamp was successfully retrieved
 * @retval     E_NOT_OK  The egress time stamp could not be retrieved
 */
...
/**
 * The implementation of EthTSyn_DataToTime.
 * @param[in]  Data  Pointer to the data of the received Ethernet frame
 * @retval     t     The time stamp converted from the data
 */
static EthTSyn_TimeStampType EthTSyn_DataToTime(const uint8* Data)
{
...
/**
 * The implementation of EthTSyn_VTToGT.
 * @param[in]  VirtualLocalTime  The virtual local time to be converted
 * @retval     t                 The time stamp converted from the virtual local time
 */
static EthTSyn_TimeStampType EthTSyn_VTToGT(StbM_VirtualLocalTimeType VirtualLocalTime)
{
    EthTSyn_TimeStampType t;
...
/**
 * The implementation of EthTSyn_TimeToData.
 * @param[in]  Time  The time stamp to be converted
 * @param[out] Data  Pointer to the data of the Ethernet frame
 * @retval     NA    The time stamp is converted to the data format
 */
static void EthTSyn_TimeToData(EthTSyn_TimeStampType Time, uint8* Data)
{
...
/* PRQA S 6010,6020 ++ */ /* VL_MTR_EthTSyn_Unchanged, VL_MTR_EthTSyn_STLIN */
/**
 * The implementation of EthTSyn_TranssmitMessage.
 * @param[in]  EthTSynMessage  The type of the message to be transmitted
 * @param[in]  PortId          Index of the port within the context of the Ethernet Interface
 */
static void EthTSyn_TranssmitMessage(EthTSynMessageType EthTSynMessage, uint16 PortId)
{
    ...
    if (port->EthTSyn_TransmissionMode == ETHTSYN_TX_ON)
    {
        ...
        if ((!hasCapturedEgress) && (FrameType == SYNC)) /* PRQA S 2992,2996 */ /* VL_EthTSyn_2992, VL_EthTSyn_2996*/
        {
            /* do nothing */
        }
        else
        {
            ...
            else
            {
                ...
                if ((ret == BUFREQ_OK) && (DataBuf != NULL_PTR) && (Length >= DataLen))
                {
                    ...
                    port->Txconfirmation =
                        (((uint64)VirtualLocalTime.nanosecondsHi) << 32u) + VirtualLocalTime.nanosecondsLo;
...
/**
 * The implementation of EthTSyn_StatusGet.
 *
 * @param[in]  PortId The port identifier.
 * @param[in]  status The status to check.
 * @return Returns true if the Ethernet Time Synchronization status is set, otherwise false.
 */
static boolean EthTSyn_StatusGet(uint16 PortId, uint8 status)
{
    boolean                 ret;
...
/**
 * The implementation of EthTSyn_CreatCorrectionField.
 *
 * @param[in]  NS    The nanoseconds to create the correction field.
 * @param[out] Data  The data buffer to fill with the correction field.
 */
static void EthTSyn_CreatCorrectionField(uint64 NS, uint8* Data)
{
    uint8 iloop;
...
/**
 * The implementation of EthTSyn_GetCorrectionField.
 *
 * @param[in]  Data The data buffer containing the correction field.
 * @return Returns the extracted correction field as an EthTSyn_TimeStampType.
 */
static EthTSyn_TimeStampType EthTSyn_GetCorrectionField(const uint8* Data)
{
    uint8                 iloop;
...
#if (ETHTSYN_MESSAGE_COMPLIANCE == STD_OFF) && (ETHTSYN_CRC_SUPPORT == STD_ON)
/**
 * The implementation of EthTSyn_TlvScanAndCRCValidation.
 *
 * @param[in]  portId       The port identifier.
 * @param[in]  ValidatedType The type of CRC validation.
 * @param[in]  Data         The data buffer to scan and validate.
 * @param[out] SGWByte     Pointer to store the SGW byte index.
 * @param[out] OFSByte     Pointer to store the OFS byte index.
 * @param[in]  frameLength  The length of the frame.
 * @param[in]  CrcFlagsRxValidated Pointer to the CRC flags configuration for validation.
 * @return Returns E_OK if validation is successful, otherwise E_NOT_OK.
 */
static Std_ReturnType EthTSyn_TlvScanAndCRCValidation(
    uint8                              portId,
    CrcValidatedType                   ValidatedType,
...
#if (ETHTSYN_HARDWARE_TIMESTAMP_SUPPORT == STD_ON)
/**
 * The implementation of EthTSyn_StbmUseEthHW.
 *
 * @param[in]  PortId The port identifier.
 * @return Returns true if the Ethernet Time Synchronization uses the hardware for time stamping, otherwise false.
 */
ETHTSYN_LOCAL_INLINE boolean EthTSyn_StbmUseEthHW(uint16 PortId)
{
...
/**
 * The implementation of EthTSyn_timeoptimal.
 *
 * @param[in]  timestamp The timestamp to optimize.
 */
...
/**
 * The implementation of EthTSyn_timeadd.
 *
 * @param[in]  time1 The first timestamp to add.
 * @param[in]  time2 The second timestamp to add.
 * @return Returns the sum of the two timestamps as an EthTSyn_TimeStampType.
 */
ETHTSYN_LOCAL EthTSyn_TimeStampType EthTSyn_timeadd(EthTSyn_TimeStampType time1, EthTSyn_TimeStampType time2)
{
...
/**
 * The implementation of EthTSyn_timeless.
 *
 * @param[in]  time1 The first timestamp to compare.
 * @param[in]  time2 The second timestamp to compare, in nanoseconds.
 * @return Returns true if time1 is less than or equal to time2, otherwise false.
 */
static boolean EthTSyn_timeless(EthTSyn_TimeStampType time1, uint64 time2)
{
    boolean ret = TRUE;
...
/**
 * The implementation of EthTSyn_TimeStampCmp.
 *
 * @param[in]  t1 The first timestamp to compare.
 * @param[in]  t2 The second timestamp to compare.
 * @return Returns -1 if t1 < t2, 0 if t1 == t2, and 1 if t1 > t2.
 */
ETHTSYN_LOCAL sint8 EthTSyn_TimeStampCmp(EthTSyn_TimeStampType t1, EthTSyn_TimeStampType t2)
{
...
/**
 * The implementation of EthTSyn_TimeStampSub.
 *
 * @param[in]  t1 The first timestamp to subtract.
 * @param[in]  t2 The second timestamp to subtract.
 * @param[out] td Pointer to store the result of the subtraction.
 */
ETHTSYN_LOCAL void EthTSyn_TimeStampSub(EthTSyn_TimeStampType t1, EthTSyn_TimeStampType t2, EthTSyn_TimeStampType* td)
{
...
/**
 * The implementation of EthTSyn_IsPortIdentityEqual.
 *
 * @param[in]  portIdentityA The first port identity to compare.
 * @param[in]  portIdentityB The second port identity to compare.
 * @return Returns true if the two port identities are equal, otherwise false.
 */
ETHTSYN_LOCAL boolean EthTSyn_IsPortIdentityEqual(const uint8* portIdentityA, const uint8* portIdentityB)
{
    return (ILib_memcmp(portIdentityA, portIdentityB, ETHTSYN_PORT_IDENTITY_LENGTH) == (uint8)0);
}

/**
 * The implementation of EthTSyn_FindPort.
 *
 * @param[in]  ctrlIdx The controller index to find the port for.
 * @return Returns a pointer to the EthTSynPortType if found, otherwise NULL_PTR.
 */
...
/**
 * The implementation of EthTSyn_MainFunctionMaster.
 *
 * @param[in]  portIndex The index of the port to process.
 * @return Returns the type of the synchronization message to send.
 */
ETHTSYN_LOCAL EthTSynMessageType EthTSyn_MainFunctionMaster(uint8 portIndex)
{
    EthTSynMessageType            syncMessageType = MSG_NONE;
    const EthTSyn_PortConfigType* portCfgPtr      = &PORT_TOCFGPORT(portIndex);
...
/**
 * The implementation of EthTSyn_InitPortStatus.
 *
 * @param[in]  port Pointer to the EthTSynPortType to initialize.
 */
static void EthTSyn_InitPortStatus(EthTSynPortType* port)
{
...
#if (ETHTSYN_DEV_ERROR_DETECT == STD_ON)
/**
 * The implementation of EthTSyn_ValidatePointer.
 *
 * @param[in]  apiId The API identifier for error reporting.
 * @param[in]  pointer The pointer to validate.
 * @return Returns true if the pointer is valid, otherwise false.
 */
static boolean EthTSyn_ValidatePointer(uint8 apiId, const void* pointer)
{
...
/**
 * The implementation of EthTSyn_ValidateInit.
 *
 * @param[in]  configPtr Pointer to the EthTSyn_ConfigType configuration structure.
 * @return Returns true if the initialization parameters are valid, otherwise false.
 */
static boolean EthTSyn_ValidateInit(const EthTSyn_ConfigType* configPtr)
{
...
/**
 * The implementation of EthTSyn_ValidateInitStatus.
 *
 * @param[in]  apiId The API identifier for error reporting.
 * @return Returns true if the EthTSyn module is initialized, otherwise false.
 */
static boolean EthTSyn_ValidateInitStatus(uint8 apiId)
{
    boolean valid = EthTSyn_Initialized;
...
/**
 * The implementation of EthTSyn_ValidateCtrlIdx.
 *
 * @param[in]  apiId The API identifier for error reporting.
 * @param[in]  ctrlIdx The controller index to validate.
 * @return Returns true if the controller index is valid, otherwise false.
 */
static boolean EthTSyn_ValidateCtrlIdx(uint8 apiId, uint8 ctrlIdx)
{
...
/**
 * The implementation of EthTSyn_ValidateTxConfirmation.
 *
 * @param[in]  ctrlIdx The controller index to validate.
 * @param[in]  pointer Pointer to the EthTSyn_TxConfirmationType structure.
 * @return Returns true if the Tx confirmation parameters are valid, otherwise false.
 */
static boolean EthTSyn_ValidateRxindication(uint8 ctrlIdx, const void* pointer)
{
...
/**
 * The implementation of EthTSyn_ValidateTransmissionMode.
 *
 * @param[in]  ctrlIdx The controller index to validate.
 * @param[in]  Mode The transmission mode to validate.
 * @return Returns true if the transmission mode is valid, otherwise false.
 */
static boolean EthTSyn_ValidateTransmissionMode(uint8 ctrlIdx, EthTSyn_TransmissionModeType Mode)
{
