...
static FUNC(Std_ReturnType, COM_CODE)
    Com_TriggerIPDUSendInternalHandle(PduIdType TxIpduId, const Com_TxIPduType* txIpduPtr);
#endif
...
static FUNC(void, COM_CODE) Com_ConfirmationProcess(const Com_TxIPduType* TxIpduPtr);
#endif
...
Com_TriggerIPDUSendHandle(PduIdType PduId)
{
    ...
    if (Com_TX_ACTIVE_EN == activeEnable)
    {
        ...
        {
            returnValue = Com_TriggerIPDUSendInternalHandle(txIpduId, txIpduPtr);
        }
    }
    else
    {
