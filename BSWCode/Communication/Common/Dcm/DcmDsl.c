...
static FUNC(Std_ReturnType, DCM_CODE) DslInternal_ProtocolStart(uint8 ProtocolCtrlId)
{
    ...
    if (DCM_PROTOCOL_INVALID == Dcm_ProtocolCtrl[ProtocolCtrlId].Dcm_ProtocolState)
    {
        ...
        {
            if (NULL_PTR == pDcmDslCallback_DCMRequestService[Index].StartProtocolFnc)
            {
                Flag = TRUE;
            }
            else
            {
                ...
                ret = (*(pDcmDslCallback_DCMRequestService[Index].StartProtocolFnc))(ProtocolIdx);
                switch (ret)
                {
...
FUNC(Std_ReturnType, DCM_CODE) DslInternal_ProtocolStop(uint8 ProtocolCtrlId)
{
    ...
    if (DCM_PROTOCOL_VALID == Dcm_ProtocolCtrl[ProtocolCtrlId].Dcm_ProtocolState)
    {
        ...
        for (Index = 0; (Index < Request_PortNum) && (FALSE == Flag); Index++)
        {
            if (NULL_PTR == pDcmDslCallback_DCMRequestService[Index].StopProtocolFnc)
            {
                Flag = TRUE;
            }
            else
            {
                ...
                ret = (*(pDcmDslCallback_DCMRequestService[Index].StopProtocolFnc))(ProtocolIdx);
                switch (ret)
                {
#ifdef RTE_E_UNCONNECTED
                case RTE_E_UNCONNECTED:
    ...
    P2VAR(PduLengthType, AUTOMATIC, DCM_VAR) bufferSizePtr)
{
    ...
    ServiceType         = Dcm_ClassifyServiceType(ProtocolId);
    len                 = Dcm_StartOfReception_GetLen(ServiceType, RxChannelCfgCtrlId, RxAddrType);
 ...
 * Sync/Async          <Synchronous>
 * Reentrancy          <Reentrant>
 * Param-Name[in]      <id:ID of DCM I-PDU that has been transmitted.
 * Param-Name[out]     <None>
 * Param-Name[in/out]  <None>
 * Return              <None>
 * PreCondition        <>
...
#include "Dcm_MemMap.h"
FUNC(void, DCM_CODE) Dcm_TxConfirmation(PduIdType id)
{
#if (STD_ON == DCM_DEV_ERROR_DETECT)
    /*The DCM module status check*/
    if (DCM_ON != (Dcm_MkCtrl.Dcm_MkState))
