 ...
 */
/*************************************************************************/
static FUNC(void, DCM_CODE) DslInternal_SetSecurityLevel(Dcm_SecLevelType NewSec, boolean isReached);
 ...
 */
/*************************************************************************/
static FUNC(void, DCM_CODE) DslInternal_UndefaultToUndefault(Dcm_SesCtrlType NewSes, boolean isReached);
 ...
 */
/*************************************************************************/
static FUNC(void, DCM_CODE) DslInternal_UndefaultTodefault(Dcm_SesCtrlType NewSes, boolean isReached);
 ...
 */
/*************************************************************************/
static FUNC(void, DCM_CODE) DslInternal_SesTranslation(Dcm_SesCtrlType NewSes, boolean isReached);
...
FUNC(void, DCM_CODE) DspInternal_DcmConfirmation_0x27(uint8 ProtocolCtrlId)
{
    ...
    {
        /*Compare key success*/
        DslInternal_SetSecurityLevel(Dcm_SecCtrl.Dcm_NewSec, FALSE);
        DslInternal_SetSecurityAccessStatus(DCM_SERVICE_IDLE);
    }
#else
    DCM_UNUSED(ProtocolCtrlId);
...
FUNC(void, DCM_CODE) DspInternal_DcmConfirmation_0x10(uint8 ProtocolCtrlId)
{
        ...
        && (0x10u == Dcm_MsgCtrl[MsgCtrlId].SID))
    {
        ...
        /*Uds 0x10 service*/
        DslInternal_SesRefresh(Dcm_SesCtrl.Dcm_NewSes, FALSE);
    }
#else
    DCM_UNUSED(ProtocolCtrlId);
#endif
...
FUNC(void, DCM_CODE) DspInternal_DcmConfirmation_0x11(uint8 ProtocolCtrlId)
{
        ...
        && (0x11u == Dcm_MsgCtrl[MsgCtrlId].SID))
    {
        if (TRUE == gAppl_UpdataOK_ResponseFlag)
        {
            ...
            DslInternal_SesRefresh(DCM_DEFAULT_SESSION, FALSE);
        }
        else
        {
...
FUNC(void, DCM_CODE) DsdInternal_ManufacturerConfirmation(uint8 ProtocolCtrlId)
{
#if (STD_ON == DCM_DSD_REQUEST_MANUFACTURER_NOTIFICATION_ENABLED)
    uint16                               PortIndex;
    uint8                                Sid     = Dcm_MkCtrl.Sid;
    uint8                                ReqType = Dcm_MkCtrl.ReqType;
    uint16                               SourceAddress;
    const Dcm_DsdCfgType*                pDcmDsdCfg   = DcmPbCfgPtr->pDcmDsdCfg;
    uint16                               Noti_PortNum = pDcmDsdCfg->DcmDsdServiceReqManufacturerNoti_PortNum;
    const Dcm_DsdServiceReqManuNotiType* pDcmDsdServiceReqManufacturerNoti =
    ...
    SourceAddress = pDcmDslMainConnection->DcmDslProtocolRxTesterSourceAddr;
#if (DCM_CPU_BYTE_ORDER == HIGH_BYTE_FIRST)
    SourceAddress = (SourceAddress << 8u) | (SourceAddress >> 8u);
#endif
...
FUNC(void, DCM_CODE) DsdInternal_SupplierConfirmation(uint8 ProtocolCtrlId)
{
    ...
    const Dcm_DslMainConnectionType* pDcmDslMainConnection =
        DcmPbCfgPtr->pDcmDslCfg->pDcmDslProtocol->pDcmDslProtocolRow[ProtocolCtrlId]
            .pDcmDslConnection->pDcmDslMainConnection;
    SourceAddress = pDcmDslMainConnection->DcmDslProtocolRxTesterSourceAddr;
    for (PortIndex = 0u; PortIndex < Noti_PortNum; PortIndex++)
    {
        (void)(*(
...
#if (                                                                                  \
    (STD_ON == DCM_UDS_SERVICE0X34_ENABLED) || (STD_ON == DCM_UDS_SERVICE0X35_ENABLED) \
    || (STD_ON == DCM_UDS_SERVICE0X36_ENABLED) || (STD_ON == DCM_UDS_SERVICE0X38_ENABLED))
 ...
 */
/*************************************************************************/
FUNC(void, DCM_CODE) DslInternal_SesRefresh(Dcm_SesCtrlType NewSes, boolean isReached)
{
    DslInternal_SesTranslation(NewSes, isReached);
...
FUNC(void, DCM_CODE) DslInternal_ProtocolStartSession(void)
{
    ...
    DslInternal_SesRefresh(Dcm_SesCtrl.Dcm_NewSes, TRUE);
...
Dcm_StartOfReception_GetLen(uint8 ServiceType, uint8 RxChannelCfgCtrlId, Dcm_DslProtocolRxAddrType RxAddrType)
{
    uint32                len        = 0;
...
{
    uint8                         SecCfgIndex;
    const Dcm_DspSecurityRowType* pCfgSecurityRow = DcmPbCfgPtr->pDcmDspCfg->pDcm_DspSecurity->pDcm_DspSecurityRow;
    const Dcm_DspSecurityRowType* pSecurityRow;
    Std_ReturnType                ret;
    ...
    if (Dcm_SecCtrl.Dcm_MaxReadoutTime < DCM_SECURITY_MAX_READOUT_TIME)
    {
        ...
        {
            pSecurityRow = &pCfgSecurityRow[SecCfgIndex];
            if ((TRUE == pSecurityRow->DcmDspSecurityAttemptCounterEnabled)
...
static FUNC(void, DCM_CODE) DslInternal_UndefaultToUndefault(Dcm_SesCtrlType NewSes, boolean isReached)
{
    ...
    DslInternal_SetSesTimingValues(NewSes);
#if (STD_ON == DCM_SECURITY_FUNC_ENABLED)
    DslInternal_SetSecurityLevel(DCM_SEC_LEV_LOCKED, isReached);
    DslInternal_SetSecurityAccessStatus(DCM_SERVICE_IDLE);
#endif
    /************************************************************
...
static FUNC(void, DCM_CODE) DslInternal_SesTranslation(Dcm_SesCtrlType NewSes, boolean isReached)
{
    ...
    else
    {
        if (DCM_DEFAULT_SESSION == NewSes)
        {
            ...
            DslInternal_UndefaultTodefault(NewSes, isReached);
        }
        else
        {
            /*from the undefault session to the undefault session */
            DslInternal_UndefaultToUndefault(NewSes, isReached);
        }
    }
}
