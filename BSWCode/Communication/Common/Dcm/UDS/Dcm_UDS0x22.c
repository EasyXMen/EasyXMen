...
static uint16           currReadDidIdx = 0x00u;
static Dcm_0x22Types    gDcm_0x22Type;
static Dcm_OpStatusType Dcm_0x22_OpStatus = DCM_INITIAL;
#if (STD_ON == DCM_DSP_DID_FUNC_ENABLED)
static uint16  Dcm_RecDid               = 0x00u;
static uint32  Dcm_ResOffset            = 0x00u;
static uint8   Dcm_DidSessionSupportNum = 0x00u;
static uint8   Dcm_NoFindDidNum         = 0x00u;
static uint8   Dcm_NoFindDidReadNum     = 0x00u;
static uint8   Dcm_RangeDidCfgIndex     = 0x00u;
static boolean Dcm_RangeDidFlag         = FALSE;
static uint16  Dcm_DidCfgIndex          = 0x00u;
static boolean Dcm_readDidSignalFlag    = FALSE;
#endif
#if (DCM_DSP_PID_FUNC_ENABLED == STD_ON)
static uint8 Dcm_MixPid       = NONE_PID;
static uint8 Dcm_noFindPidNum = 0x00u;
#endif
    ...
    if (Find == TRUE)
    {
        ...
        if (E_OK == ret)
        {
            ...
            for (iloop = 0; (iloop < DDDidNum) && (E_OK == ret); iloop++)
            {
                ...
                {
                    ret = Dcm_UdsAssembleResponse_0x2CSubDeal(OpStatus, DDDid, &ResOffset, pNrc);
                }
    ...
    else
    {
        ...
        if (SesRefNum != 0u)
        {
            for (Index = 0; (Index < SesRefNum) && (FALSE == Flag); Index++)
            {
                ...
                {
                    Flag = TRUE;
                    if (Dcm_0x22_OpStatus == DCM_INITIAL)
                    {
    ...
    P2CONST(Dcm_0x22Types, AUTOMATIC, DCM_VAR) Dcm_0x22Type)
{
    ...
    uint32                      ResOffset        = *Dcm_0x22Type->ResOffset;
#if ((NVM_ENABLE == STD_ON) && (DCM_DATA_USE_BLOCK_ID_ENABLED == STD_ON))
    uint32 ResOffsetRead = *Dcm_0x22Type->ResOffset;
#endif
    *Dcm_0x22Type->readDidSignalFlag = FALSE;
    ...
    for (DidSignalIndex = 0; DidSignalIndex < DidSignalNum; DidSignalIndex++)
    {
        ...
        if (USE_ECU_SIGNAL == pDspDidData->DcmDspDataUsePort)
        {
            ...
            ResOffset += (uint32)pDspDidData->DcmDspDataSize;
            ...
            if (USE_BLOCK_ID == pDspDidData->DcmDspDataUsePort)
        {
            ...
            *Dcm_0x22Type->readDidSignalFlag = TRUE;
            ...
            {
                (void)NvM_ReadBlock(pDspDidData->DcmDspDataBlockId, (void*)&Dcm_Channel[ResOffsetRead]);
                ResOffsetRead += (uint32)pDspDidData->DcmDspDataSize;
            }
...
#if (((NVM_ENABLE == STD_ON) && (DCM_DATA_USE_BLOCK_ID_ENABLED == STD_ON)) || (DCM_DATA_ECU_SIGNAL_ENABLED == STD_ON))
    if (ret == E_OK)
    {
        *Dcm_0x22Type->ResOffset = ResOffset;
    }
    ...
    P2VAR(Dcm_NegativeResponseCodeType, AUTOMATIC, DCM_VAR) pNrc)
{
    ...
    for (DidSignalIndex = 0; (DidSignalIndex < DidSignalNum) && (E_OK == ret); DidSignalIndex++)
    {
        ...
        if (E_OK == ret)
        {
            ...
            {
                if ((dataoffset + didResOffset - pDcmChannelCfg->offset) > pDcmChannelCfg->Dcm_DslBufferSize)
                {
                    /**< buffer overflow*/
                ...
                {
                    ret = pDspDidData->DcmDspDataReadFnc(OpStatus, &Dcm_Channel[DidSignalPos + didResOffset], pNrc);
                }
            }
        }
    }
#if (TRUE == DCM_PAGEDBUFFER_ENABLED)
    if (((didResOffset + dataoffset - pDcmChannelCfg->offset) > pDcmChannelCfg->Dcm_DslBufferSize))
    {
        ...
        if (((dataoffset + 2u) <= pDcmChannelCfg->Dcm_DslBufferSize) && (E_OK == ret))
        {
            if (Dcm_PageBufferData.BufferOverFlowed == FALSE)
            {
                ...
                {
                    Dcm_PageBufferData.FilledSize = didResOffset - 2u - pDcmChannelCfg->offset;
                    /** init response offset */
                    Dcm_PageBufferData.ResOffset = pDcmChannelCfg->offset;
                }
                else
                {
                    Dcm_PageBufferData.NextFilledSize = didResOffset - 2u - pDcmChannelCfg->offset;
                }
            }
        }
        else
    ...
    if (ret == DCM_PENDING)
    {
        Dcm_PageBufferData.ResOffset = didResOffset;
    }
#endif
    if (ret == E_OK)
    {
    ...
    P2VAR(Dcm_NegativeResponseCodeType, AUTOMATIC, DCM_VAR) ErrorCode)
{
    ...
    else
    {
        ...
        else
        {
            ...
            {
                ret =
                    DspInternalUDS0x22_DidSessionCheck(DidInfoCfgIndex, Dcm_0x22Type->DidSessionSupportNum, ErrorCode);
            }
            if (E_NOT_OK == ret)
            {
                ret = E_OK;
    ...
    *ErrorCode = DCM_E_REQUESTOUTOFRANGE;
    ...
    else
    {
        ...
        if (TRUE == Dcm_PageBufferData.PagedBufferStarted)
        {
            ...
            Dcm_ResOffset = Dcm_PageBufferData.ResOffset; // buffer offset position when pending
        }
        else
#endif
        {
            if (OpStatus == DCM_INITIAL)
            {
                Dcm_Channel[Offset] = 0x62u; /*response SID*/
                Dcm_ResOffset       = Offset + 1u;
#if (DCM_DSP_PID_FUNC_ENABLED == STD_ON)
                Dcm_noFindPidNum           = 0;
                gDcm_0x22Type.MixPid       = &Dcm_MixPid;
                gDcm_0x22Type.noFindPidNum = &Dcm_noFindPidNum;
#endif
                Dcm_DidSessionSupportNum           = 0;
                Dcm_NoFindDidReadNum               = 0;
                Dcm_NoFindDidNum                   = 0;
                gDcm_0x22Type.NoFindDidReadNum     = &Dcm_NoFindDidReadNum;
                gDcm_0x22Type.DidSessionSupportNum = &Dcm_DidSessionSupportNum;
                gDcm_0x22Type.NoFindDidNum         = &Dcm_NoFindDidNum;
                gDcm_0x22Type.ResOffset            = &Dcm_ResOffset;
                gDcm_0x22Type.pRangeDidCfgIndex    = &Dcm_RangeDidCfgIndex;
                gDcm_0x22Type.pDidCfgIndex         = &Dcm_DidCfgIndex;
                gDcm_0x22Type.pRangeDidFlag        = &Dcm_RangeDidFlag;
                gDcm_0x22Type.readDidSignalFlag    = &Dcm_readDidSignalFlag;
                currReadDidIdx                     = 0u;
                Dcm_0x22_OpStatus                  = DCM_INITIAL;
            }
            if (OpStatus == DCM_CANCEL)
            {
                Dcm_0x22_OpStatus = DCM_CANCEL;
            }
            ReqOffset = 1;
            Index0    = 0u;
        }
        ...
        )
        {
            for (; (Index0 < DidNum) && ((E_OK == ret))
                 ...
                 Index0++)
            {
                Dcm_RecDid = (uint16)(((uint16)pMsgContext->pReqData[ReqOffset]) << 8u)
                             | ((uint16)(pMsgContext->pReqData[ReqOffset + 1u]));
                ReqOffset = ReqOffset + 2u;

                if ((Index0 < currReadDidIdx)
#if (TRUE == DCM_PAGEDBUFFER_ENABLED)
                    && (FALSE == Dcm_PageBufferData.PagedBufferStarted)
#endif
                )
                {
                    continue;
...
#if (DCM_DSP_PID_FUNC_ENABLED == STD_ON)
                if ((Dcm_RecDid >= 0xF400u) && (Dcm_RecDid <= 0xF8FFu))
                {
                    Dcm_DidSessionSupportNum++;
                    gDcm_0x22Type.RecDid = Dcm_RecDid;
                    ret                  = DspInternalUDS0x22_ObdDidDeal(ProtocolCtrlId, &gDcm_0x22Type, ErrorCode);
                }
                if (((Dcm_MixPid == NEED_CALL_UDS_API) || (Dcm_MixPid == NONE_PID)) && ((E_OK == ret)))
#endif
                {
                    gDcm_0x22Type.RecDid = Dcm_RecDid;
                    ret =
                        DspInternalUDS0x22_NonObdDidDeal(Dcm_0x22_OpStatus, ProtocolCtrlId, &gDcm_0x22Type, ErrorCode);
                }
                if (ret == E_OK)
                {
                    Dcm_0x22_OpStatus = DCM_INITIAL;
                    currReadDidIdx    = currReadDidIdx + 1u;
                }
                /*if the requested DID reading is not completed, the subsequent DID reading is terminated*/
                if (DCM_E_PENDING == ret)
                {
                    Dcm_0x22_OpStatus = DCM_PENDING;
                    break;
                }
#if (TRUE == DCM_PAGEDBUFFER_ENABLED)
            ...
            if (((Dcm_NoFindDidNum == DidNum) || (Dcm_NoFindDidReadNum == DidNum)
#if (DCM_DSP_PID_FUNC_ENABLED == STD_ON)
                 || (Dcm_noFindPidNum == DidNum)
#endif
                 || (Dcm_DidSessionSupportNum == 0u))
                && (E_OK == ret))
            {
        ...
        if ((E_OK == ret) && (OpStatus != DCM_CANCEL))
        {
#if (TRUE == DCM_PAGEDBUFFER_ENABLED)
            if ((Dcm_PageBufferData.PagedBufferStarted == FALSE) && (Dcm_PageBufferData.BufferOverFlowed == TRUE))
            {
                ...
                ret = Dcm_UDS0x22_StartPagedProcessing(ProtocolCtrlId, Dcm_ResOffset, ErrorCode);
            }
            ...
            else if ((Dcm_PageBufferData.PagedBufferStarted == TRUE) && (Index0 == DidNum))
            {
                ...
                Dcm_PageBufferData.IsNextFilled     = TRUE;
                Dcm_PageBufferData.NextFilledSize   = Dcm_ResOffset - Offset;
                Dcm_PageBufferData.IsBufferOccupied = TRUE;
                Dcm_PageBufferData.IsRecord         = FALSE;
...
#endif
            {
                pMsgContext->ResMaxDataLen = (Dcm_MsgLenType)Dcm_ResOffset - Offset;
                pMsgContext->ResDataLen    = (Dcm_MsgLenType)Dcm_ResOffset - Offset;
