...
static Std_ReturnType
    DspInternalUDS0x2E_DidSecCheck(uint16 DidInfoCfgIndex, P2VAR(Dcm_NegativeResponseCodeType, AUTOMATIC, DCM_VAR) pNrc)
{
    const Dcm_DspDidWriteType* pDspDidWrite;
    Std_ReturnType             ret = E_NOT_OK;
    uint8                      Index;
    ...
    P2VAR(Dcm_NegativeResponseCodeType, AUTOMATIC, DCM_VAR) ErrorCode)
{
    ...
    for (DidSignalIndex = 0; (DidSignalIndex < DidSignalNum) && (ret == E_OK); DidSignalIndex++)
    {
        ...
        {
            didSize += pDspDidData->DcmDspDataSize;
        }
        else
        {
            if ((DidSignalIndex + 1u) == DidSignalNum)
            {
                isDynamic = TRUE;
            }
            else
    ...
    expectedLen = 3u + (uint32)didSize;
    if ((E_OK == ret)
        && (((expectedLen != reqDataLen) && (!isDynamic))
            || ((isDynamic)
                && ((expectedLen >= reqDataLen) || ((expectedLen + pDspDidData->DcmDspDataSize) < reqDataLen)))))
    {
    ...
    P2VAR(Dcm_NegativeResponseCodeType, AUTOMATIC, DCM_VAR) ErrorCode)
{
    ...
    if (E_OK == ret)
    {
        ...
        if (pDcmDspDidInfo[*Dcm_0x2EType->DidInfoCfgIndex].pDcmDspDidWrite == NULL_PTR)
        {
    ...
    P2VAR(Dcm_NegativeResponseCodeType, AUTOMATIC, DCM_VAR) ErrorCode)
{
    ...
    if (E_OK == ret)
    {
        ...
        {
            ret = DspInternalUDS0x2E_DidWriteDataLength(DidCfgIndex, pMsgContext->ReqDataLen, ErrorCode);
            if (E_OK == ret)
            {
