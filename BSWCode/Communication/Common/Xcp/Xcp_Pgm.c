...
#define XCP_START_SEC_VAR_CLEARED_8
#include "Xcp_MemMap.h"
static VAR(uint8, XCP_VAR) Xcp_FlsBuffer[XCP_FLS_DATA_BUFFER_SIZE]; /*PRQA S 3218*/ /*VL_Xcp_3218*/
#define XCP_STOP_SEC_VAR_CLEARED_8
#include "Xcp_MemMap.h"
...
static FUNC(void, XCP_CODE) Xcp_FlsBlockProgramPending(void)
{
    ...
    case XCP_IF_OK:
        ...
        if ((XCP_FLS_MIN_WRITE_SIZE + Xcp_BlockBufferPos) <= Xcp_BlockBufferLen)
        {
            ...
            {
                Xcp_FlsBufAlignPtr[cnt] = Xcp_BlockBuffer[Xcp_BlockBufferPos + cnt]; /*PRQA S 2841*/ /*VL_Xcp_2841*/
            }
...
FUNC(void, XCP_CODE) Xcp_ProgramInit(void)
{
    ...
    cnt = 0u;
    while (((uint32)(&Xcp_FlsBuffer[cnt]) % 4u) != 0u) /* PRQA S 0306 */ /* VL_Xcp_0306 */
    {
        cnt++;
    }
...
#if (XCP_ADDR_GRANULARITY != XCP_AG_DWORD)
    NumOfBytes = 2u + (programNumberOfData * XCP_AG); /*PRQA S 2985*/ /*VL_QAC_2985_2986*/
#else
...
FUNC(void, XCP_CODE) Xcp_Program(void)
{
    ...
    if (
#if (XCP_ADDR_GRANULARITY != XCP_AG_DWORD)
        (Xcp_CmdLength != (2u + (Xcp_CmdBuffer[1u] * XCP_AG))) /*PRQA S 2985*/ /*VL_QAC_2985_2986*/
#else
    ...
    if (((Std_ReturnType)E_OK == Xcp_CheckPgmStatus()) && (XCP_PGM_PROGRAMMING != Xcp_PgmStauts))
    {
        ...
        if ((Std_ReturnType)E_OK == result)
        {
            ...
            if (Xcp_BlockBufferLen >= (XCP_FLS_MIN_WRITE_SIZE - Xcp_FlsBufferSize))
            {
                ...
                Xcp_BlockBufferPos += cnt; /*PRQA S 2986*/ /*VL_QAC_2985_2986*/
                /* set fls buffer size to min write size */
                Xcp_FlsBufferSize = XCP_FLS_MIN_WRITE_SIZE;
                switch (Xcp_Program_AccessMode)
                {
